...
def _schema_version(data):
    return str(data.get("schema_version") or data.get("schemaVersion") or "")


def _is_v2_assessment(data):
    return _schema_version(data).startswith("2")


def validate_v2_structure(path, data):
    errors = []
    missing = sorted(V2_REQUIRED_KEYS - set(data))
    if missing:
        errors.append(f"{path.name}: v2 missing required keys: {', '.join(missing)}")
    ...
    if not isinstance(rules, list) or not rules:
        ...
        for index, rule in enumerate(rules):
            for key in ("rule_id", "category", "recommendation", "required_evidence", "evidence_factors"):
                if key not in rule:
                    errors.append(f"{path.name}: rules[{index}] missing {key}")
    ...
    if not isinstance(cases, list) or not cases:
        ...
        for index, case in enumerate(cases):
            for key in ("case_id", "rule_id", "properties_required", "metrics_required", "cost_required", "evidence_factors"):
                if key not in case:
                    errors.append(f"{path.name}: cases[{index}] missing {key}")

    metrics = (data.get("azure_metrics") or {}).get("metrics") or []
    if not metrics:
        errors.append(f"{path.name}: azure_metrics.metrics must be non-empty")

    fields = (data.get("cost_management") or {}).get("fields") or []
    field_names = {item.get("field") for item in fields if isinstance(item, dict)}
    for required_field in ("billed_mtd", "retail_monthly"):
        if required_field not in field_names:
            errors.append(f"{path.name}: cost_management missing field {required_field}")
...
def validate_structure(path, data):
    errors = []
    if _is_v2_assessment(data):
        return validate_v2_structure(path, data)
