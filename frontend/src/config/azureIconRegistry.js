 ...
 */
import { API_PATH_TO_CANONICAL, COUNT_KEY_TO_CANONICAL } from './resourceApiPaths';
...
export const ICON_COMPONENTS = {
  ...
  resourceGraph: AzResourceGroupList,
...
export const PAGE_ICON_KEYS = {
  cloudExplorer: 'resourceGraph',
  actionCentre: 'recommendations',
  dashboard: 'dashboard',
...
export const ROUTE_ICON_KEYS = {
  '/': 'recommendations',
  '/dashboard': 'dashboard',
  '/explorer': 'resourceGraph',
  '/action-centre': 'recommendations',
  '/costs': 'costManagement',
...
export function iconKeyForApiPath(apiPath) {
  const direct = API_PATH_KEYS[apiPath];
  if (direct) return direct;
  const canonical = API_PATH_TO_CANONICAL[String(apiPath || '').trim().toLowerCase().replace(/\/+$/, '')];
  if (!canonical) return null;
  const legacySlug = Object.entries(COUNT_KEY_TO_CANONICAL).find(([, ct]) => ct === canonical)?.[0];
  return legacySlug ? API_PATH_KEYS[`/resources/${legacySlug}`] || null : null;
}
