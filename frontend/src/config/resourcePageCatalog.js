...
import { apiPathForCanonical } from './resourceApiPaths';
...
export function buildPerTypeResourcePages() {
  return Object.fromEntries(
    Object.entries(PER_TYPE_RESOURCE_PAGES).map(([id, page]) => {
      const canonical = PER_TYPE_API_PATH_TO_CANONICAL[page.apiPath]
        || PER_TYPE_API_PATH_TO_CANONICAL[`/resources/${id}`];
      return [
        id,
        {
          ...page,
          iconKey: id,
          apiPath: canonical ? apiPathForCanonical(canonical) : page.apiPath,
        },
      ];
    }),
  );
