...
describe('resourceFindingsUtils', () => {
  ...
  it('does not fall back when index is ready and empty', () => {
    expect(resolveResourceFindings(resource, [], { indexReady: true })).toEqual([]);
    expect(resourceHasFindings(resource, [], { indexReady: true })).toBe(false);
  });
