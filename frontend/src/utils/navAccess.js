...
const SUPERUSER_ONLY_PATHS = new Set(['/explorer']);

export function normalizeNavPath(path) {
  if (!path || path === '/') return '/action-centre';
  const base = path.split('?')[0];
  const normalized = base.endsWith('/') && base.length > 1 ? base.slice(0, -1) : base;
  if (normalized.startsWith('/action-centre')) return '/action-centre';
  if (normalized.startsWith('/explorer')) return '/explorer';
  if (normalized.startsWith('/optimization-hub')) return '/action-centre';
  return normalized;
}
...
export const NAV_ACCESS_SUBGROUPS = {
  ...
  resources: RESOURCE_NAV_CATALOG.map((group) => ({
    id: group.id,
    label: group.label,
...
export function canViewNavPath(path, allowedPaths, { isSuperuser = false } = {}) {
  ...
  if (SUPERUSER_ONLY_PATHS.has(normalized) && !isSuperuser) return false;
  ...
  if (sections.length) {
    if (sectionsAllowed(sections, allowed)) return true;
    return false;
  }
