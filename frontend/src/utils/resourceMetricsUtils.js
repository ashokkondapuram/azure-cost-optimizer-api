...
const FACT_LABELS = {
  ...
  storage_pct: 'Storage utilization',
};
...
const UNIT_FORMATTERS = {
  ...
  count: (num) => Math.round(num).toLocaleString('en-US'),
...
function inferUnitFromFactKey(factKey) {
  ...
  if (isThroughputFactKey(factKey)) return 'bytes_per_sec';
  if (key.endsWith('_iops') || key.includes('operations/sec')) return 'count';
  if (isMemoryFactKey(factKey)) return 'bytes';
  if (isBytesFactKey(factKey)) return 'bytes';
...
export function formatFactValue(factKey, value, unit) {
  ...
  const propertyFormatted = formatPropertyMetricValue(factKey, num);
  if (propertyFormatted) return propertyFormatted;
  ...
  if (isMemoryFactKey(factKey)) return formatMemory(num);
  if (isBytesFactKey(factKey)) return formatBytes(num);
  if (isThroughputFactKey(factKey)) return formatThroughput(num);
  if (isCountFactKey(factKey) || resolvedUnit === 'count') {
    return UNIT_FORMATTERS.count(num);
  }
