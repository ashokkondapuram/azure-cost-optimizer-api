...
/** Unwrap API enum objects ({ code }) to a scalar without display fallbacks. */
export function unwrapScalar(value) {
  if (value == null) return value;
  if (typeof value === 'object' && !Array.isArray(value)) {
    const inner = value.code ?? value.label ?? value.name ?? value.value;
    if (inner != null && inner !== '') return unwrapScalar(inner);
    return '';
  }
  return value;
}
