...
describe('resourceMetricsUtils', () => {
  ...
  test('formats available memory bytes', () => {
    expect(formatFactValue('avg_available_memory_bytes', 62_262_717_653.0656)).toBe('58.0 GB');
  });

  test('formats pod count as integer without decimals', () => {
    expect(formatFactValue('pod_count', 3220.23)).toBe('3,220');
    expect(formatFactValue('pod_count', 3220.23, 'count')).toBe('3,220');
  });

  test('formats disk size properties in GB', () => {
    expect(formatFactValue('diskSizeGB', 128)).toBe('128 GB');
  });
