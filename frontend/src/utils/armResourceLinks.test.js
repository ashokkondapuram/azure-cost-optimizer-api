...
describe('armResourceLinks', () => {
  ...
  test('builds inventory inspect links', () => {
    const link = inventoryInspectLink(DISK_ID);
    expect(link).toContain('/disks?');
    expect(link).toContain('search=cso-54802-pgcore');
    expect(link).toContain('inspect=1');
    expect(link).toContain('section=advanced-analysis');
    expect(link).toContain(`resourceId=${encodeURIComponent(DISK_ID)}`);
  ...
  test('parses VM and VMSS disk host attachments', () => {
    ...
    expect(vm?.inventoryLink).toContain('/vms?');
    ...
    expect(vmssInstance?.inventoryLink).toContain('/vmss?');
