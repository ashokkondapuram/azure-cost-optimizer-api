...
describe('authRedirect', () => {
  ...
  it('defaults to dashboard', () => {
    expect(postLoginPath(new URLSearchParams(), null)).toBe('/dashboard');
  });
  ...
  it('rejects open redirects and API paths', () => {
    expect(postLoginPath(new URLSearchParams('next=%2F%2Fevil.com'), null)).toBe('/dashboard');
    expect(postLoginPath(new URLSearchParams('next=%2Fresources%2Fvms'), null)).toBe('/dashboard');
    expect(postLoginPath(new URLSearchParams('next=%2Flogin'), null)).toBe('/dashboard');
