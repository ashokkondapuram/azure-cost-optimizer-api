...
/**
 * ZafinityOps brand mark — infinity loop + orbital node (Zafin × continuous ops).
 */
export default function ZafinityOpsLogo({
  size = 32,
  className = '',
  title = 'ZafinityOps',
}) {
  const uid = useId().replace(/:/g, '');
  const bgId = `zo-bg-${uid}`;
  const shineId = `zo-shine-${uid}`;
...
export function ZafinityOpsWordmark({ className = '' }) {
  return (
    <div className={`sidebar-logo__text${className ? ` ${className}` : ''}`}>
      Zafinity<span>Ops</span>
    </div>
  );
}
