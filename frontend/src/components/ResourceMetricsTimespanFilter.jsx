...
import { METRIC_TIMESPAN_OPTIONS, coerceMetricTimespan } from '../utils/metricsTimespanUtils';
...
export default function ResourceMetricsTimespanFilter({
  ...
  const selected = coerceMetricTimespan(value);
  return (
    <label
    ...
    >
      ...
      <select
        ...
        value={selected}
        onChange={(event) => onChange(coerceMetricTimespan(event.target.value))}
