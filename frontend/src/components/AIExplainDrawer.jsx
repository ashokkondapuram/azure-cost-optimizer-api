...
export default function AIExplainDrawer({ open, onClose, resource, findingType, title }) {
  ...
  return (
    <div className={`ai-explain-drawer${open ? ' ai-explain-drawer--open' : ''}`}>
      <div className="ai-explain-drawer__backdrop" onClick={onClose} />
      <div className="ai-explain-drawer__panel">
        <div className="ai-explain-drawer__accent" />
        <div className="ai-explain-drawer__head">
          <div>
            <h2 className="ai-explain-drawer__title">
              <Sparkles size={16} style={{ color: 'var(--primary)', marginRight: '0.4rem', verticalAlign: 'middle' }} />
              AI Explanation
            </h2>
            {title && <p className="ai-explain-drawer__subtitle">{title}</p>}
          </div>
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            {data && (
              <button type="button" className="ai-explain-drawer__close" onClick={copyExplanation} title="Copy">
                {copied ? <Check size={14} /> : <Copy size={14} />}
              </button>
            )}
            <button type="button" className="ai-explain-drawer__close" onClick={onClose} title="Close">
              <X size={14} />
            </button>
          </div>
        </div>
          ...
          {!loading && data && (
            <>
              ...
              <div className="ai-section">
                <div className="ai-section__head"><ChevronRight size={13} /> Why is this flagged?</div>
                <p className="ai-section__body">{data.why}</p>
              </div>

              <div className="ai-section ai-section--act">
                <div className="ai-section__head"><ChevronRight size={13} /> Impact if you act</div>
                <p className="ai-section__body">{data.impact_act}</p>
              </div>

              <div className="ai-section ai-section--ignore">
                <div className="ai-section__head"><AlertTriangle size={13} /> Impact if ignored</div>
                <p className="ai-section__body">{data.impact_ignore}</p>
              </div>

              <div className="ai-section">
                <div className="ai-section__head"><ChevronRight size={13} /> How to fix</div>
                <pre className="ai-section__code">{data.how_to}</pre>
              </div>

              <p className="ai-disclaimer">AI-generated explanation. Verify before taking action.</p>
            </>
          )}
        </div>
      </div>
