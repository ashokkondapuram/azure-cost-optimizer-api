...
export default function AdvisorTableCell({
  ...
  if (waiting) {
    return <span className="resource-table__empty" title="Loading Advisor findings">…</span>;
  }
  if (isError && !engineIcons.length && !advisorIcons.length) {
    return (
      <span className="resource-table__empty resource-table__empty--warn" title="Could not load Advisor findings">
        !
      </span>
    );
  }
  ...
  if (!advisorIcons.length && !engineIcons.length) {
    if (subscriptionHasAdvisor || subscriptionHasFindings) {
      return <span className="resource-table__empty" title="No Advisor findings for this resource">—</span>;
    }
    return (
      <span
        className="advisor-table-cell advisor-table-cell--sync-hint text-sm"
        title="Sync Azure Advisor and run analysis in Sync center"
      >
        Sync
      </span>
    );
