...
export default function useDashboardCostPeriod(storageKey = 'finops-dashboard-cost-period') {
  ...
  const [storedPeriod, setPeriod] = usePersistedState(storageKey, DEFAULT_DASHBOARD_COST_PERIOD);
  const period = isValidDashboardCostPeriod(storedPeriod, periodOptions)
    ? storedPeriod
    : DEFAULT_DASHBOARD_COST_PERIOD;
