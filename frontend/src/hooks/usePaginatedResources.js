...
export default function usePaginatedResources({
  ...
  const wantsMetrics = includeMetrics || String(apiPath || '').includes('/disks');
  const wantsCosts = includeCosts !== false;
  const query = useInfiniteQuery({
    queryKey: [apiPath, subscription, dataSource, 'paged', pageSize, includeProperties, inventoryOnly, wantsMetrics, wantsCosts],
    ...
    queryFn: ({ pageParam }) => {
      const params = {
        ...
        ...(wantsMetrics ? { include_metrics: true } : {}),
        ...(wantsCosts ? { include_costs: true } : { include_costs: false }),
      };
      if (pageParam?.cursor) {
