...
describe('createResourceMatcher', () => {
  ...
  test('matches when row type is canonical (DB inventory rows)', () => {
    const matchAks = createResourceMatcher({
      apiPath: '/resources/aks',
      canonicalType: 'containers/aks',
      armTypeHint: 'managedclusters',
    });
    expect(matchAks({ type: 'containers/aks' }, '')).toBe(true);
  });
