...
import { diskAssessmentPropertyGroupDefs } from '../../../disks/diskAssessment';
...
describe('compute-disk diskUtils', () => {
  ...
  test('builds disk property tiles for drawer summary from assessment schema', () => {
    ...
    expect(tiles.find((t) => t.key === 'properties-diskSizeGB')?.value).toBe('128 GB');
    expect(tiles.find((t) => t.key === 'properties-diskState')?.tone).toBe('warn');
  });

  test('getDiskPropertyValue resolves nested ARM paths', () => {
    const resource = {
      sku: { name: 'Premium_LRS' },
      location: 'eastus',
      properties: { diskSizeGB: 512, tier: 'P50' },
    };
    expect(getDiskPropertyValue(resource, 'sku.name')).toBe('Premium_LRS');
    expect(getDiskPropertyValue(resource, 'properties.diskSizeGB')).toBe(512);
    expect(getDiskPropertyValue(resource, 'properties.tier')).toBe('P50');
    expect(getDiskPropertyValue(resource, 'location')).toBe('eastus');
  });

  test('buildDiskPropertiesSections returns all assessment groups with non-empty values', () => {
    const sections = buildDiskPropertiesSections({
      sku: { name: 'Premium_LRS' },
      location: 'eastus',
      properties: {
        diskSizeGB: 128,
    ...
    const groupIds = sections.map((s) => s.group);
    expect(groupIds).toContain('configuration');
    expect(groupIds).toContain('capacity');
    expect(groupIds).toContain('attachment');
    expect(groupIds).toContain('security');
    expect(diskAssessmentPropertyGroupDefs()).toHaveLength(4);
    const labels = sections.flatMap((s) => s.items.map((i) => i.label));
    expect(labels).toContain('Attached to');
    expect(labels).toContain('Encryption settings');
    expect(labels).not.toContain('Last synced');
  });
  ...
  test('reads PascalCase ARM disk properties', () => {
    ...
    expect(tiles.find((t) => t.key === 'properties-diskSizeGB')?.value).toBe('256 GB');
    expect(tiles.find((t) => t.key === 'properties-diskIOPSReadWrite')?.value).toBe('1,100');
    expect(tiles.find((t) => t.key === 'properties-diskMBpsReadWrite')?.value).toBe('125 MB/s');
    expect(tiles.find((t) => t.key === 'properties-managedBy')?.value).toBe('vm-1');
  });
  ...
  test('maps VMSS instance attachments', () => {
    ...
    expect(tiles.find((t) => t.key === 'properties-managedBy')?.value).toBe('app-vmss / instance 2');
  });
  ...
  test('builds usage tiles from monitor metrics bundle', () => {
    ...
    expect(usage[0].value).toBe('13 IOPS avg · 40 IOPS max · 1,100 provisioned');
    expect(usage[1].value).toBe('3 IOPS avg · 8 IOPS max');
