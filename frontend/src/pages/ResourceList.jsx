...
import { humanizeAzureRegion, formatCurrency, formatDate } from '../utils/format';
import { resourceTotalCost } from '../utils/costCurrency';
import useColumnConfig from '../hooks/useColumnConfig';
import useResponsiveView from '../hooks/useResponsiveView';
import ResourceCard, { ResourceCardGroup } from '../components/responsive/ResourceCard';
import { exportAllResourcesCSV } from '../utils/exporters';
...
import { iconForRow, iconForApiPath, serviceDisplayNameForRow } from '../config/assetIcons';
...
export default function ResourceList({ title, apiPath, iconSrc, iconKey }) {
  ...
  const rows = displayRows.filter((r) => {
    if (!matchResourceRow(r, search, [
      ...
      (row) => humanizeAzureRegion(row.location || row.region),
    ])) return false;
  ...
  const renderCell = (row, col) => {
    ...
    if (col.key === 'azureServiceName') {
      const text = serviceDisplayNameForRow(row);
      ...
      <PageHeader title={title} iconKey={pageIcon}>
        <div className="page-header-actions__row">
          ...
          <button
            ...
            onClick={() => exportAllResourcesCSV(sortedRows, `${apiPath.replace(/\//g, '-')}-${new Date().toISOString().slice(0, 10)}.csv`)}
          ...
          {!isLoading && !isError && rows.length > 0 && isMobile && (
            <div className="resource-card-list">
              {groupedRows.map(([rg, groupRows]) => {
                ...
                return (
                  <ResourceCardGroup key={rg} resourceGroup={rg} rows={groupRows}>
                    {groupRows.map((row) => {
                      const rid = resourceId(row);
                      return (
                        <ResourceCard
                          ...
                          row={row}
                          nameLabel={toDisplayText(rowValue(row, nameCol))}
                          stateText={stateCol ? resolveRowState(row) : ''}
                          showState={!!stateCol}
                          showCost={!!costCol}
                          currency={currency}
                          apiPath={apiPath}
                          ...
                          savings={savingsByResource.get(rid) || 0}
                          indexReady={indexReady}
                          advisorRecommendations={lookupAdvisorForResource(advisorByResourceId, row)}
                          advisorIndexReady={advisorIndexReady && !advisorIndexLoading}
                          advisorIndexError={advisorIndexError}
                          subscriptionHasAdvisor={advisorByResourceId.size > 0}
                          subscriptionHasFindings={byResourceId.size > 0}
