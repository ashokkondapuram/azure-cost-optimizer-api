...
import { diskSku, diskStateLabel, diskAttachmentSummary, diskAttachmentTypeLabel, diskProvisionedIopsLabel, diskProvisionedMbpsLabel } from '../it-services/compute-disk';
import {
...
export default function VirtualMachines() {
  ...
  const diskState = (d) => diskStateLabel(d);
        ...
        {tab === 'disks' && (
          ...
          filteredDisks.length === 0 ? (
            ...
            <div className={resourceTableWrapClass(filteredDisks.length)}>
              <table className="table resource-table">
                <thead><tr><th>Name</th><th>Size</th><th>SKU</th><th>Provisioned IOPS</th><th>Throughput</th><th>State</th><th>Attached to</th><th>{costColumnLabel(currency)}</th><th>Advisor</th><th>Cost signals</th><th>Findings</th><th>Location</th><th>Resource group</th></tr></thead>
                <tbody>
                  {groupByResourceGroup(filteredDisks).map(([rgName, groupRows]) => (
                    <React.Fragment key={rgName}>
                      ...
                      {groupRows.map((d) => {
                        ...
                        const attachmentLabel = diskAttachmentSummary(d);
                        const attachmentType = diskAttachmentTypeLabel(d);
                        return (
                          <tr key={d.id || d.name} className="resource-table__row" onClick={() => setSelected(d)}>
                            ...
                            <td style={{ fontVariantNumeric: 'tabular-nums' }}>{diskProvisionedIopsLabel(d)}</td>
                            <td style={{ fontVariantNumeric: 'tabular-nums' }}>{diskProvisionedMbpsLabel(d)}</td>
                            <td><span className={`badge ${state === 'Unattached' ? 'badge-critical' : 'badge-low'}`}>{state}</span></td>
                            <td title={attachmentType || undefined}>
                              <span style={{ color: attachmentLabel === 'Unattached' ? 'var(--text3)' : undefined }}>
                                {attachmentLabel}
                              </span>
                            </td>
