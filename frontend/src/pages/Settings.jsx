...
import {
  ...
  Trash2, Users, Activity, LayoutGrid,
} from 'lucide-react';
...
const ALL_TAB_IDS = new Set(
  TAB_SECTIONS.flatMap((section) => section.tabs.map((tab) => tab.id)),
);

function resolveSettingsTab(tabId, isSuperuser) {
  if (!tabId || !ALL_TAB_IDS.has(tabId)) return 'users';
  if (tabId === 'nav-access' && !isSuperuser) return 'users';
  return tabId;
}
...
const TAB_META = {
  ...
  kubernetes: {
    ...
    description: 'Connect AKS clusters, deploy the utilization agent, and configure ingestion.',
  },
  runtime: {
...
function RuntimeStatusCompact({ status }) {
  ...
  const { database, cors, deployment, azure } = status;
    ...
    <div className="settings-form-stack">
      ...
      <div className="settings-grid">
        <FieldRow
          ...
          source={data.default_subscription_id_source}
          hint="When set, sync workers and the sidebar default to this subscription. Use Settings → Sync from Azure to list available IDs, or set AZURE_DEFAULT_SUBSCRIPTION_ID in Docker."
        >
          <input
            type="text"
...
function KubernetesClustersPanel({ defaultSubscriptionId }) {
  const qc = useQueryClient();
  const [subscriptionId, setSubscriptionId] = useState(defaultSubscriptionId || '');
  const [panelMsg, setPanelMsg] = useState({ type: '', text: '' });
  ...
  const connectedQuery = useQuery({
    queryKey: ['k8s-connected-clusters'],
    queryFn: fetchConnectedK8sClusters,
  });

  const discoverMut = useMutation({
    mutationFn: () => discoverK8sClusters(subscriptionId.trim()),
    onSuccess: () => setPanelMsg({ type: 'success', text: 'Clusters loaded from Azure.' }),
    onError: (err) => setPanelMsg({ type: 'error', text: getErrorMessage(err, 'Could not discover clusters.') }),
  });

  const connectMut = useMutation({
    mutationFn: (cluster) => connectK8sCluster({
      subscription_id: subscriptionId.trim(),
      resource_group: cluster.resource_group,
      cluster_name: cluster.name,
    }),
    onSuccess: (res) => {
      setPanelMsg({ type: 'success', text: `Connected to ${res.cluster?.cluster_name || 'cluster'}.` });
      qc.invalidateQueries({ queryKey: ['k8s-connected-clusters'] });
    },
    onError: (err) => setPanelMsg({ type: 'error', text: getErrorMessage(err, 'Could not connect cluster.') }),
  });

  const deployMut = useMutation({
    mutationFn: (clusterId) => deployK8sAgent(clusterId),
    onSuccess: (res) => {
      setPanelMsg({ type: 'success', text: res.message || 'Agent deployed.' });
      qc.invalidateQueries({ queryKey: ['k8s-connected-clusters'] });
    },
    onError: (err) => setPanelMsg({ type: 'error', text: getErrorMessage(err, 'Could not deploy agent.') }),
  });

  const connected = connectedQuery.data?.clusters || [];
  const discovered = discoverMut.data?.clusters || [];
  const connectedKeys = new Set(
    connected.map((c) => `${c.subscription_id}:${c.resource_group}:${c.cluster_name}`.toLowerCase()),
  );

  return (
    <div className="settings-form-stack">
      <h3 className="settings-subsection-title">Connected clusters</h3>
      <p className="setting-field__hint">
        Uses your Azure connection (Settings → Azure connection). The service principal needs
        Reader on the subscription and Azure Kubernetes Service Cluster User Role on the cluster.
      </p>

      <div className="settings-grid">
        <FieldRow label="Subscription ID" hint="Subscription that hosts your AKS clusters.">
          <input
            type="text"
      ...
      <div className="settings-panel__actions">
        <button
          ...
          disabled={!subscriptionId.trim() || discoverMut.isPending}
          onClick={() => discoverMut.mutate()}
        >
          <RefreshCw size={14} className={discoverMut.isPending ? 'spin' : ''} /> Discover clusters
        </button>
      </div>

      <StatusBanner type={panelMsg.type} message={panelMsg.text} />
            ...
            <tbody>
              {connected.map((cluster) => (
                <tr key={cluster.id}>
                  ...
                  <td>{cluster.agent_status || 'not_deployed'}</td>
                  <td>
                    <button
                      type="button"
                      className="btn btn-secondary btn-sm"
                      disabled={deployMut.isPending}
                      onClick={() => deployMut.mutate(cluster.id)}
                    >
                      Deploy agent
                    </button>
                  </td>
      ...
      {discovered.length > 0 && (
        <>
          <h3 className="settings-subsection-title">Discovered in Azure</h3>
          <div className="settings-table-wrap">
            <table className="data-table">
              <thead>
                <tr>
              ...
              <tbody>
                {discovered.map((cluster) => {
                  const key = `${subscriptionId}:${cluster.resource_group}:${cluster.name}`.toLowerCase();
                  const isConnected = connectedKeys.has(key);
                  return (
                    <tr key={cluster.arm_id || key}>
                      <td>{cluster.name}</td>
                      <td>{cluster.resource_group}</td>
                      <td>{cluster.kubernetes_version || '—'}</td>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => connectMut.mutate(cluster)}
                        >
...
function KubernetesForm({ data, onChange, azureDraft }) {
  return (
    <div className="settings-form-stack">
      <div className="settings-grid">
        <FieldRow
        ...
        <FieldRow
          label="Agent API URL"
          source={data.agent_api_url_source}
          hint="Base URL the in-cluster agent calls (no trailing slash). Required when the API is not reachable at the dashboard host."
        >
          <input
            type="text"
            value={data.agent_api_url || ''}
            onChange={(e) => onChange('agent_api_url', e.target.value)}
            placeholder="https://finops.example.com"
            autoComplete="off"
          />
        </FieldRow>
        <FieldRow
          label="Agent image"
          source={data.agent_image_source}
          hint="Optional pre-built image (e.g. myregistry.azurecr.io/finops/k8s-agent:1.0). Leave blank to deploy via python:3.12-alpine."
        >
          <input
            type="text"
            value={data.agent_image || ''}
            onChange={(e) => onChange('agent_image', e.target.value)}
            placeholder="finops/k8s-agent:1.0"
            autoComplete="off"
          />
        </FieldRow>
        <FieldRow label="Poll interval (seconds)" source={data.poll_interval_seconds_source}>
          <input
            type="number"
            min={15}
            max={3600}
            value={data.poll_interval_seconds ?? 60}
            onChange={(e) => onChange('poll_interval_seconds', Number(e.target.value))}
          />
        </FieldRow>
      </div>
      <KubernetesClustersPanel defaultSubscriptionId={azureDraft?.default_subscription_id || ''} />
    </div>
  );
}
...
function DataManagementPanel({ subscription, subscriptionLabel }) {
  ...
  const [clearMode, setClearMode] = useState('synced');
  ...
  const clearMut = useMutation({
    mutationFn: () => {
      const params = { mode: clearMode };
      if (clearMode === 'synced' && scope === 'subscription' && subscription) {
        params.subscription_id = subscription;
      }
      return clearDatabaseData(params);
    },
    onSuccess: (res) => {
      const deleted = res.deleted || {};
      const total = Object.values(deleted).reduce((sum, n) => sum + (n || 0), 0);
      const restartNote = res.mode === 'all'
        ? ' Restart the app or sign in again — default users are recreated on startup.'
        : ' Run Sync from Azure to reload inventory.';
      setResultMsg({
        type: 'success',
        text: `Cleared ${total.toLocaleString()} rows.${restartNote}`,
      });
  ...
  const canConfirm = clearMode === 'all'
    ? confirmText.trim().toUpperCase() === 'CLEAR ALL'
    : confirmText.trim().toUpperCase() === 'CLEAR';
  const scopeLabel = clearMode === 'all'
    ? 'all database tables'
  ...
  return (
    <div className="settings-form-stack">
      <div className="alert alert--warning">
        ...
        <span>
          {clearMode === 'all'
            ? 'This removes every row from every database table, including user accounts, Azure connection settings, engine rules, and all synced data. You will need to sign in again after the app recreates default users.'
            : 'This removes synced Azure inventory, costs, budgets, optimization findings, advisor data, actions, and run history from the local database. User accounts, engine rules, and system settings are kept.'}
        </span>
      </div>

      <div className="setting-field">
        <div className="setting-field__label">Clear type</div>
        <div className="auth-method-picker" role="radiogroup" aria-label="Clear type">
          <button
            type="button"
            role="radio"
            aria-checked={clearMode === 'synced'}
            className={`auth-method-picker__option${clearMode === 'synced' ? ' auth-method-picker__option--active' : ''}`}
            onClick={() => setClearMode('synced')}
          >
            <span className="auth-method-picker__label">Synced data</span>
            <span className="auth-method-picker__desc">
              Inventory, costs, findings, advisor, actions, and analysis history
            </span>
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={clearMode === 'all'}
            className={`auth-method-picker__option${clearMode === 'all' ? ' auth-method-picker__option--active' : ''}`}
            onClick={() => setClearMode('all')}
          >
            <span className="auth-method-picker__label">All tables</span>
            <span className="auth-method-picker__desc">
              Wipe every table, including users, settings, and engine configuration
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="setting-field">
        ...
        <ul className="settings-list-hint">
          {clearMode === 'all' ? (
            <>
              <li>All synced Azure and optimization data</li>
              <li>User accounts and login history</li>
              <li>Azure connection, database, and application settings</li>
              <li>Engine rule profiles and thresholds</li>
            </>
          ) : (
            <>
              <li>Resource inventory, pricing profiles, and dependencies</li>
              <li>Cost snapshots, budgets, and utilization history</li>
              <li>Optimization findings, actions, advisor, and run history</li>
              <li>Assessment pipeline, maintenance cache, and K8s snapshots</li>
              <li>Subscription and token caches</li>
            </>
          )}
        </ul>
      </div>
      ...
      <div className="settings-panel__actions">
        <button
          ...
          disabled={clearMode === 'synced' && scope === 'subscription' && !subscription}
          onClick={() => { setConfirmOpen(true); setConfirmText(''); setResultMsg({ type: '', text: '' }); }}
        >
          <Trash2 size={14} /> {clearMode === 'all' ? 'Clear all tables' : 'Clear database'}
        </button>
      </div>
      ...
      {confirmOpen && (
        <div className="modal-overlay" onClick={() => !clearMut.isPending && setConfirmOpen(false)} role="presentation">
          ...
          >
            <h2 id="clear-db-title" className="modal-title">
              {clearMode === 'all' ? 'Clear all tables?' : 'Clear database?'}
            </h2>
            <p style={{ margin: '0 0 1rem', color: 'var(--text2)', lineHeight: 1.5 }}>
              You are about to clear <strong>{scopeLabel}</strong>.
              This cannot be undone. Type{' '}
              <strong>{clearMode === 'all' ? 'CLEAR ALL' : 'CLEAR'}</strong> to confirm.
            </p>
            <input
              type="text"
              value={confirmText}
              onChange={(e) => setConfirmText(e.target.value)}
              placeholder={clearMode === 'all' ? 'Type CLEAR ALL' : 'Type CLEAR'}
              autoComplete="off"
...
export default function Settings() {
  ...
  const qc = useQueryClient();
  const { subscription, subscriptionLabel } = useSubscriptionLabel();
  const [tab, setTab] = useState(() => resolveSettingsTab(searchParams.get('tab'), isSuperuser));
  ...
  useEffect(() => {
    const nextTab = resolveSettingsTab(searchParams.get('tab'), isSuperuser);
  ...
  const subLabel = subscriptionLabel;
