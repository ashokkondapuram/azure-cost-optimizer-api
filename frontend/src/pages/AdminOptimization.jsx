...
import { QueryErrorState } from '../components/QueryStates';
...
export default function AdminOptimization() {
  ...
  const handleAdvisorSync = async () => {
    ...
    try {
      ...
      showToast(
        fetched > 0
          ? `Synced ${stored.toLocaleString()} Azure Advisor findings.`
          : 'Azure Advisor returned no active findings.',
        { variant: 'success' },
      );
    } catch (err) {
  ...
  const runBatch = async () => {
    ...
    try {
      ...
      showToast(
        job.recovered
          ? 'Analysis started. The server was slow to respond — track progress in the bar at the top.'
          : scoped
  ...
  return (
    <div className="page-shell admin-optimization-page">
      ...
      <PageHero
        ...
        eyebrow="Operations"
        title={subscription ? 'Sync and analyze' : 'Sync center'}
        subtitle={
          fullAnalysisBlocked
            ? fullAnalysisHint
      ...
      <section className="page-section card opt-actions-card">
        ...
        <div className="opt-toolbar__actions">
          ...
          <Link to="/action-centre" className="btn btn--ghost">
            Open action centre <ChevronRight size={14} />
          </Link>
        </div>
        </div>
      </section>
      ...
      {subscription && isError && (
        <QueryErrorState
          error={error}
          onRetry={refetch}
          title="Could not load optimization overview."
        />
      )}
          ...
          {!overview.last_analyzed_at && totals.resource_count > 0 && (
            <div className="page-callout card">
              <p>
                ...
                <strong>Re-run analysis</strong> to populate findings.
              </p>
            </div>
          )}
