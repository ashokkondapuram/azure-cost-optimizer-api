  ...
  const saveMut = useMutation({
    ...
    onSuccess: async () => {
      ...
      try {
        ...
        setReanalyzeNote(result?.message || 'Findings are refreshing in the background.');
      } catch (err) {
        setReanalyzeNote(
  ...
  return (
    <div className="page-shell engine-config-page">
      ...
      >
        ...
        {reanalyzing && (
          <span className="text-muted text-sm">Refreshing recommendations…</span>
        )}
        {!reanalyzing && reanalyzeNote && (
          <span className="text-muted text-sm" style={{ maxWidth: 280 }}>{reanalyzeNote}</span>
        )}
        <button
          type="button"
