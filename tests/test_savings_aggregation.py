...
from app.savings_aggregation import (
    ...
    unified_savings_by_resource_engine,
)
...
def test_unified_savings_by_resource_engine_dedupes_overlapping_rules():
    rid = "/subscriptions/s/rg/providers/microsoft.compute/virtualmachines/vm1"
    findings = [
        _finding("VM_IDLE", 200.0, resource_id=rid),
        _finding("VM_OVERSIZE", 80.0, resource_id=rid),
    ]
    by_resource = unified_savings_by_resource_engine(findings)
    assert by_resource[rid.lower()] == 200.0
