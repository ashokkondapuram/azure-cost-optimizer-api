...
VMSS_INSTANCE_ID = (
    ...
    "Microsoft.Compute/virtualMachineScaleSets/aks-system-vmss/virtualMachines/0"
)
VMSS_ID = (
    "/subscriptions/s/resourceGroups/MC_rg/providers/"
...
def test_monitor_arm_type_detects_vmss_instance():
    assert monitor_arm_type(VMSS_INSTANCE_ID) == "microsoft.compute/virtualmachinescalesets/virtualmachines"
    assert monitor_arm_type(VMSS_ID) == "microsoft.compute/virtualmachinescalesets"
    inst_profile = get_monitor_profile(VMSS_INSTANCE_ID)
    parent_profile = get_monitor_profile(VMSS_ID)
    assert inst_profile is not None
    assert parent_profile is not None
    assert inst_profile.monitor_arm_type != parent_profile.monitor_arm_type
