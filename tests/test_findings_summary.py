...
def test_build_findings_summary_counts_status_severity_and_type(db_session):
    ...
    assert summary["by_category_ordered"][0]["key"] == "COMPUTE"
    assert summary["by_severity_ordered"][0]["key"] == "HIGH"
...
def test_build_findings_summary_exposes_unified_per_resource_map(db_session):
    resource_id = "/subscriptions/sub-1/resourceGroups/rg/providers/Microsoft.Compute/virtualMachines/vm-1"
    db_session.add_all([
        _finding(id="f1", resource_id=resource_id, rule_id="VM_IDLE", estimated_savings_usd=200.0),
        _finding(id="f2", resource_id=resource_id, rule_id="VM_OVERSIZE", estimated_savings_usd=80.0),
    ])
    db_session.commit()

    summary = build_findings_summary(db_session, "sub-1")

    assert summary["total_estimated_savings_usd"] == 200.0
    assert summary["raw_total_estimated_savings_usd"] == 280.0
    assert summary["savings_by_resource_usd"][resource_id.lower()] == 200.0
