...
from app.cost_utils import summarize_cost_response, subscription_totals_summary
...
def test_normalize_query_response_aliases_actual_cost_column():
    raw = {
        "properties": {
            "columns": [
                {"name": "Cost"},
    ...
    out = normalize_query_response(raw)
    names = [c["name"] for c in out["properties"]["columns"]]
    assert names == ["PreTaxCost", "CostUSD", "Currency"]
    summary = summarize_cost_response(out)
...
def test_subscription_totals_summary_prefers_top_level_fields():
    resp = {
        "pretax_total": 1234.56,
        "cost_usd_total": 987.65,
        "billing_currency": "CAD",
        "properties": {
            "columns": [{"name": "Currency"}, {"name": "PreTaxCost"}, {"name": "CostUSD"}],
            "rows": [[999.0, 1.0, 2.0]],
        },
    }
    summary = subscription_totals_summary(resp)
    assert summary["pretax_total"] == 1234.56
    assert summary["cost_usd_total"] == 987.65


def test_cost_request_401_retries_with_fresh_token(monkeypatch):
    """401 recovery must not reuse a stale pinned bearer from arm_auth_context."""
    ...
    def fake_request(method, url, headers, *, params=None, payload=None, throttle_label="cost_api", _retried=False):
        token = headers.get("Authorization", "")
        attempts.append(token)
        if len(attempts) == 1:
            raise AzureAPIError(401, "Unauthorized", "token expired")
        return {"properties": {"columns": [], "rows": []}}

    monkeypatch.setattr(azure_cost, "_request", fake_request)
    monkeypatch.setattr(azure_cost, "_pause_between_cost_queries", lambda *_a, **_k: None)
    monkeypatch.setattr(azure_cost, "clear_arm_cache", lambda: None)

    with patch("app.azure_cost.refresh_azure_token") as refresh, \
         patch(
             "app.azure_cost.fresh_azure_headers",
             return_value={"Authorization": "Bearer fresh-token", "Content-Type": "application/json"},
         ), \
         arm_auth_context(token="stale-pinned"):
        azure_cost._cost_request(
            "POST",
            "https://management.azure.com/subscriptions/x/providers/Microsoft.CostManagement/query",
            {"Authorization": "Bearer stale-pinned"},
            payload={"type": "ActualCost"},
        )

    refresh.assert_called_once()
    assert attempts[0] == "Bearer stale-pinned"
    assert attempts[1] == "Bearer fresh-token"
