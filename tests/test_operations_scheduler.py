...
def test_get_scheduler_status_shows_component_mode(monkeypatch):
    ...
    assert status["analysis"]["enabled"] is False
    assert status["analysis"]["chained_after_component_sync"] is True
    assert status["pipeline"]["interval_enabled"] is False
    assert "rollout_observation" not in status
    get_settings.cache_clear()


def test_scheduled_analysis_disabled_when_component_pipeline_chains(monkeypatch):
    monkeypatch.setenv("APP_ENV", "production")
    monkeypatch.delenv("SCHEDULED_ANALYSIS_ENABLED", raising=False)
    monkeypatch.delenv("SCHEDULED_COMPONENT_SYNC_ENABLED", raising=False)
    from app.settings import get_settings

    get_settings.cache_clear()
    assert sched.scheduled_analysis_enabled() is False
    assert sched.scheduled_analysis_interval_enabled() is False
    assert sched.scheduled_pipeline_interval_enabled() is False
    get_settings.cache_clear()


def test_scheduled_analysis_explicit_override(monkeypatch):
    monkeypatch.setenv("SCHEDULED_ANALYSIS_ENABLED", "true")
    assert sched.scheduled_analysis_enabled() is True
