...
def test_bucket_to_types_covers_vms():
    ...
    assert "vmss" not in BUCKET_TO_TYPES


def test_empty_buckets_keeps_vmss_slot_empty():
    from app.analysis import empty_buckets

    assert empty_buckets().get("vmss") == []


def test_load_buckets_for_keys_vmss_excluded_from_inventory():
    from app.analysis import load_buckets_for_keys
