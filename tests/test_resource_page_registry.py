...
from app.resource_api_paths import API_PATH_TO_CANONICAL as FULL_API_PATH_TO_CANONICAL
...
def test_loganalytics_api_path_maps_to_canonical_type():
    assert API_PATH_TO_CANONICAL["/resources/monitoring/loganalytics"] == "monitoring/loganalytics"
    assert FULL_API_PATH_TO_CANONICAL["/resources/loganalytics"] == "monitoring/loganalytics"
...
def test_all_inventory_pages_have_unique_ids_and_paths():
    ...
    for page in pages:
        assert api_path_for_page(page) == f"/resources/{page.canonical_type}"
