...
def test_run_sub_engines_sets_exclude_inventory_facts_on_evidence(monkeypatch):
    ...
    if findings:
        assert findings[0].evidence.get("exclude_inventory_facts") is True
        assert "resource_elements" not in findings[0].evidence
        assert "sub_engine" not in findings[0].evidence


def test_extended_engine_uses_sub_engines(monkeypatch):
    monkeypatch.setenv("LEGACY_SUB_ENGINES_ENABLED", "true")
    eng = ExtendedOptimizationEngine()
    vm = _vm("vm-sub")
    result = eng.analyze(
    ...
    for finding in result["findings"]:
        ...
        if finding.get("resource_id", "").lower() == vm["id"].lower():
            assert ev.get("exclude_inventory_facts") is True
            assert "sub_engine" not in ev
            break
