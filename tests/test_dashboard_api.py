...
from app.dashboard.api import _cost_vs_utilization, utilization_by_resource_type
...
from app.data_store.enrichment_registry import ensure_all_enrichment_tables
...
def test_dashboard_overview_aggregates_sections(db_session):
    ...
    assert findings_kpi["href"] == "/dashboard#findings-insights"
    ...
    health_panel = overview["portal"]["panels"]["resource_health_status"]
    health_keys = {segment["key"] for segment in health_panel["segments"]}
    assert health_keys == {"healthy", "degraded", "unavailable", "unknown"}
    kpi_ids = {k["id"] for k in overview["portal"]["kpis"]}
    assert "resources_degraded" in kpi_ids
    assert "resources_unavailable" in kpi_ids
...
def test_utilization_by_resource_type_from_enrichment(db_session):
    rid = "/subscriptions/sub-1/resourcegroups/rg/providers/microsoft.compute/virtualmachines/vm-1"
    ...
    upsert_metrics(
        ...
        snap,
        {"avg_cpu_pct": 12.5},
        metrics_payload={"ok": True, "facts": {"avg_cpu_pct": 12.5}, "metrics": [], "derived": []},
    )
    db_session.commit()
...
def test_advisor_findings_kpi_in_portal(db_session):
    ...
    assert advisor_kpi["href"] == "/dashboard#findings-insights"
...
def test_cost_vs_utilization_uses_resource_spend_and_enrichment(db_session):
    from datetime import date

    rid = "/subscriptions/sub-1/resourceGroups/rg/providers/Microsoft.Compute/virtualMachines/vm-1"
    month = date.today().strftime("%Y-%m")
    _add_resource(db_session, rid=rid, name="vm-1", cost_usd=120.0)
    db_session.flush()
    row = (
        db_session.query(ResourceSnapshot)
        .filter(ResourceSnapshot.resource_id == rid.lower())
        .one()
    )
    ...
    upsert_metrics(
        ...
        row,
        {"avg_cpu_pct": 8.0},
        metrics_payload={"ok": True, "facts": {"avg_cpu_pct": 8.0}, "metrics": [], "derived": []},
    )
    db_session.commit()

    items = _cost_vs_utilization(db_session, "sub-1", [], [], timeframe="MonthToDate")
    assert len(items) == 1
    assert items[0]["name"] == "vm-1"
    assert items[0]["cost"] == 150.0
    assert items[0]["utilization_pct"] == 8.0


def test_underutil_outliers_includes_metric_status(db_session):
    rid = "/subscriptions/sub-1/resourceGroups/rg/providers/Microsoft.Compute/virtualMachines/vm-2"
    db_session.add(OptimizationFinding(
        id=str(uuid.uuid4()),
        run_id="run-1",
        rule_id="NETWORK_VNET_EMPTY",
        ...
        evidence_json=json.dumps({
            "optimization_metrics": {
                "performance": [
                    {"id": "avg_cpu_pct", "formatted": "6.0%", "status": "underutilized"},
                ],
            },
        }),
    ))
    ...
    result = list_underutil_outliers(db_session, "SUB-1", limit=5)
    assert result["count"] == 1
    assert result["items"][0]["resource_name"] == "vm-2"
