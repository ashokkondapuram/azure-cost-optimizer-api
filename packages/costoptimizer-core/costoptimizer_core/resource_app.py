...
def create_resource_service(config: ServiceConfig) -> FastAPI:
    ...
    from app.resource_store import get_resources_db, get_resources_db_page, resource_cost_map_from_db
    ...
    def get_resource(
        ...
        sub = subscription_id.strip().lower()
        rows = get_resources_db(db, sub, config.canonical_type, include_properties=True)
