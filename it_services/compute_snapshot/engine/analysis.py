...
from it_services.compute_snapshot.engine.optimization_rules import evaluate_snapshot_archive_candidate
...
def analyze_snapshots(engine, subscription_id: str, snapshots: list[dict], cost_by_resource: dict[str, float]) -> list[ExtendedFinding]:
    ...
    archive_rule = engine.rules.get("SNAPSHOT_ARCHIVE_EXTENDED")
    now = datetime.now(timezone.utc)
    for snapshot in snapshots:
        rid = snapshot.get("id") or ""
        savings = resource_cost(cost_by_resource, rid)
        _append_metrics_draft(
            out, engine, subscription_id, snapshot, archive_rule,
            evaluate_snapshot_archive_candidate(snapshot, savings, archive_rule),
        )
