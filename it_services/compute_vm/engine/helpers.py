...
from app.vm_cost_decision import align_vm_target_sku, compute_vm_resize_pricing, resolve_vm_cost_decision
...
def vm_sizing_pricing(
    ...
    *,
    ...
    """Return (savings_usd or None, pricing_evidence) using run-rate × retail ratio."""
    savings, pricing = compute_vm_resize_pricing(
        vm, current_sku, suggested_sku, monthly_cost=monthly_cost,
    )
    if pricing.get("pricing_status") != "available":
        return None, pricing
    return savings, pricing


def merge_advisor_vm_target(
    sizing,
    *,
    current_sku: str,
    advisor: AdvisorVmTarget | None,
):
    """Prefer Azure Advisor target SKU when a cost resize recommendation exists."""
    return align_vm_target_sku(sizing, current_sku=current_sku, advisor=advisor)
...
def emit_vm_sizing_finding(
    ...
    *,
    ...
    advisor_target: AdvisorVmTarget | None = None,
) -> ExtendedFinding | None:
    decision = resolve_vm_cost_decision(
        vm=vm,
        current_sku=sku,
        engine_sizing=sizing,
        advisor=advisor_target,
        monthly_cost=monthly_cost,
    )
    if not decision or not decision.sizing or not decision.target_sku:
        return None

    sizing = decision.sizing
    if sizing.action not in {"downgrade", "upgrade", "cross_family"}:
        return None
    ...
    savings = decision.monthly_savings
    ...
    if current_retail is not None and suggested_retail is not None:
        run_rate = pricing.get("monthly_run_rate_usd")
        if pricing.get("savings_basis") == "monthly_run_rate" and run_rate:
            savings_detail = (
                f" Monthly run-rate ${run_rate:,.2f}/mo × retail ratio "
                f"({sku} ~${current_retail:,.2f}/mo → {decision.target_sku} ~${suggested_retail:,.2f}/mo) "
                f"(est. savings ${savings:,.2f}/mo)."
            )
        else:
            savings_detail = (
                f" Azure retail pricing: {sku} ~${current_retail:,.2f}/mo → "
                f"{decision.target_sku} ~${suggested_retail:,.2f}/mo "
                f"(est. savings ${savings:,.2f}/mo)."
            )
    if decision.sku_agreement == "agree":
        savings_detail += " Engine agrees with Azure Advisor target."
    elif decision.sku_agreement == "disagree":
        savings_detail += " Engine suggests a different SKU — review Advisor alignment."
    elif decision.sku_source == "azure_advisor":
        savings_detail += " Target SKU from Azure Advisor."
    ...
    return apply_vm_rightsizing_severity(
        ...
        recommendation=(
            f"{verb} {sku} → {decision.target_sku}. "
        ...
        evidence={
            **util.as_dict(),
            "vm_size": sku,
            "suggested_sku": decision.target_sku,
