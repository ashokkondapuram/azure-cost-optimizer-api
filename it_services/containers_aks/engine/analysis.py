...
from it_services.containers_aks.engine.helpers import (
    aks_supported_minors,
    aks_version_catalog,
    is_node_auto_provisioning_enabled,
)
...
def analyze_aks(engine, subscription_id: str, clusters: list[dict], aks_node_pools: dict[str, list], node_metrics: dict[str, dict], cost_by_resource: dict[str, float]) -> list[ExtendedFinding]:
    ...
    for cluster in clusters:
        ...
        for pool in pools:
            ...
            if consolidation_rule and consolidation_rule.enabled and not nap_enabled and count >= 3:
            ...
            if autoscale_rule and autoscale_rule.enabled and not nap_enabled and not asc and count > autoscale_rule.node_count_min:
                out.append(engine._finding(
                    rule=autoscale_rule,
                    subscription_id=subscription_id,
                    resource=cluster,
            ...
            if count > 0 and idle_rule.enabled and not nap_enabled:
