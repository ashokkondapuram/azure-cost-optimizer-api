...
def is_node_auto_provisioning_enabled(cluster_props: dict | None) -> bool:
    """True when AKS Node Auto Provisioning (NAP) is enabled (mode Auto)."""
    profile = (cluster_props or {}).get("nodeProvisioningProfile") or {}
    return str(profile.get("mode") or "").strip().lower() == "auto"
