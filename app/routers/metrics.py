...
from app.validators import coerce_metric_timespan, ensure_subscription_known
...
def _metric_timespan(timespan: Any = Query("P7D", description="Azure Monitor lookback")) -> str:
...
@router.get("/utilization-series", tags=["Monitor"],
         summary="Historical utilization points for one resource metric")
def metrics_utilization_series(
    request: Request,
    subscription_id: str = Query(...),
    resource_id: str = Query(..., description="Full ARM resource ID"),
    metric_name: str = Query(..., description="Monitor fact key, e.g. avg_cpu_pct"),
    timespan: str = Depends(_metric_timespan),
    limit: int = Query(90, ge=2, le=180),
...
def metrics_resource_auto(
    ...
    resource_id: str = Query(..., description="Full ARM resource ID"),
    timespan: str = Depends(_metric_timespan),
    refresh: bool = Query(False, description="Bypass DB cache and refetch from Azure Monitor"),
    db: Session = Depends(get_db),
):
    require_authenticated_user(request)
    from app.metrics_api import fetch_metrics_for_resource
    return fetch_metrics_for_resource(resource_id, timespan=timespan, db=db, refresh=refresh)


@router.post("/sync", tags=["Monitor"],
         summary="Fetch and persist metrics for synced inventory (admin)")
def metrics_sync_subscription(
    request: Request,
    subscription_id: str = Query(...),
    ...
    return run_inventory_metrics_worker(db, subscription_id)
