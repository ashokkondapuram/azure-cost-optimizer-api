...
def admin_optimization_overview(
    ...
    try:
        ...
        log.exception("admin_optimization_overview_failed", subscription_id=subscription_id)
...
@router.post("/data/clear", summary="Clear synced data or all database tables (admin only)")
def clear_database_data(
    ...
    mode: str = Query(
        "synced",
        description="synced = operational Azure data only; all = every table including users and settings",
    ),
    db: Session = Depends(get_db),
):
    ...
    require_admin_user(request)
    if mode not in ("synced", "all"):
        raise HTTPException(status_code=400, detail="mode must be 'synced' or 'all'")
    try:
        return {"status": "ok", **clear_database(db, subscription_id=subscription_id, mode=mode)}
    except Exception as exc:
