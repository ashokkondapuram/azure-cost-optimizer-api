...
from app.validators import coerce_bool_dict_map
...
class KubernetesSettingsIn(BaseModel):
    ...
    agent_api_url: Optional[str] = Field(
        None,
        description="Public API base URL the in-cluster agent calls (no trailing slash)",
    )
    agent_image: Optional[str] = Field(
        None,
        description="Optional pre-built agent image; leave blank to deploy via python:3.12-alpine + ConfigMap",
    )
    poll_interval_seconds: Optional[int] = Field(None, ge=15, le=3600)
...
class NavAccessPolicyIn(BaseModel):
    ...
    @classmethod
    def _coerce_roles(cls, value):
        return coerce_bool_dict_map(value)
