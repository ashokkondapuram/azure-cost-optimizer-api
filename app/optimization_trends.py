...
PIPELINE_WORKFLOW_STATUSES = ("proposed", "approved", "executed")
...
def _pipeline_actions_by_status(actions: list[OptimizationAction]) -> dict[str, int]:
    counts = {status: 0 for status in PIPELINE_WORKFLOW_STATUSES}
    for action in actions:
        status = (action.workflow_status or "proposed").strip().lower()
        if status in counts:
            counts[status] += 1
    return counts
...
def get_optimization_trends(db: Session, subscription_id: str) -> dict[str, Any]:
    ...
    pipeline_actions_by_status = _pipeline_actions_by_status(actions)
    ...
    return {
        ...
        "pipeline_actions_by_status": pipeline_actions_by_status,
