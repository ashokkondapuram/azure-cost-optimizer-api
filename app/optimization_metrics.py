...
METRIC_DEFS: dict[str, dict[str, Any]] = {
    ...
    "health_probe_count": {"category": "performance", "id": "health_probes", "label": "Health probes", "unit": "probes", "format": "number"},
    ...
    "egress_bytes": {"category": "performance", "id": "egress", "label": "Data egress", "unit": "bytes", "format": "number", "status_fn": lambda v: "high" if v is not None and float(v) >= 107374182400 else None},
    ...
    "snat_utilization_pct": {
        "category": "performance",
        "id": "snat_utilization_pct",
        "label": "SNAT utilization",
        "unit": "%",
        "format": "percent",
        "status_fn": lambda v: "high" if v is not None and float(v) >= 80 else None,
    },
    "snat_port_usage_pct": {
        "category": "performance",
        "id": "snat_port_usage_pct",
        "label": "SNAT port usage",
        "unit": "%",
        "format": "percent",
        "status_fn": lambda v: "high" if v is not None and float(v) >= 70 else None,
    },
    "used_snat_ports": {"category": "performance", "id": "used_snat_ports", "label": "Used SNAT ports", "unit": "ports", "format": "number"},
    "allocated_snat_ports": {"category": "performance", "id": "allocated_snat_ports", "label": "Allocated SNAT ports", "unit": "ports", "format": "number"},
    "backend_availability_pct": {
        "category": "performance",
        "id": "backend_availability_pct",
        "label": "Backend availability",
        "unit": "%",
        "format": "percent",
        "status_fn": lambda v: "low" if v is not None and float(v) < 50 else None,
    },
    "byte_count_peak": {"category": "performance", "id": "byte_count_peak", "label": "Peak traffic volume", "unit": "bytes", "format": "number"},
    "billed_capacity_units": {
        "category": "performance",
        "id": "billed_capacity_units",
        "label": "Billed capacity units",
        "unit": "CU",
        "format": "number",
    },
    "cu_utilization_pct": {
        "category": "performance",
        "id": "cu_utilization_pct",
        "label": "Capacity unit utilization",
        "unit": "%",
        "format": "percent",
        "status_fn": lambda v: (
            "high" if v is not None and float(v) >= 80 else ("low" if v is not None and float(v) < 30 else None)
        ),
    },
    "current_connections": {
        "category": "performance",
    ...
    "failed_request_count": {
        ...
        "unit": "requests",
        "format": "number",
        "status_fn": lambda v: "high" if v is not None and float(v) > 0 else None,
    },
    "packet_drop_count": {"category": "performance", "id": "packet_drop_count", "label": "Dropped packets", "unit": "packets", "format": "number"},
    "datapath_availability_pct": {
        "category": "performance",
        "id": "datapath_availability_pct",
        "label": "Datapath availability",
        "unit": "%",
        "format": "percent",
    },
    ...
    "disk_throughput_utilization_pct": {
        "category": "performance",
        "id": "disk_throughput_utilization",
    ...
    "normalized_ru_pct": {
        "category": "performance",
        "id": "normalized_ru_pct",
        "label": "Normalized RU consumption",
        "unit": "%",
        "format": "percent",
        "status_fn": lambda v: "high" if v is not None and float(v) >= 80 else ("low" if v is not None and float(v) < 20 else None),
    },
    "normalized_ru_peak_pct": {
        "category": "performance",
        "id": "normalized_ru_peak_pct",
        "label": "Peak normalized RU consumption",
        "unit": "%",
        "format": "percent",
        "status_fn": lambda v: "high" if v is not None and float(v) >= 95 else None,
    },
    "provisioned_throughput": {"category": "performance", "id": "provisioned_throughput", "label": "Provisioned throughput", "unit": "RU/s", "format": "number"},
    "data_usage_bytes": {"category": "performance", "id": "data_usage_bytes", "label": "Data usage", "unit": "bytes", "format": "number"},
    "index_usage_bytes": {"category": "performance", "id": "index_usage_bytes", "label": "Index usage", "unit": "bytes", "format": "number"},
    "document_count": {"category": "performance", "id": "document_count", "label": "Document count", "unit": "count", "format": "number"},
    "replication_latency_ms": {"category": "performance", "id": "replication_latency_ms", "label": "Replication latency", "unit": "ms", "format": "number"},
    "server_latency_ms": {"category": "performance", "id": "server_latency_ms", "label": "Server-side latency", "unit": "ms", "format": "number"},
    "ru_skew_ratio": {"category": "performance", "id": "ru_skew_ratio", "label": "RU skew ratio", "unit": "ratio", "format": "number"},
    "index_to_data_ratio": {"category": "performance", "id": "index_to_data_ratio", "label": "Index to data ratio", "unit": "ratio", "format": "number"},
    "avg_item_bytes": {"category": "performance", "id": "avg_item_bytes", "label": "Average item size", "unit": "bytes", "format": "number"},
    "api_type": {"category": "performance", "id": "api_type", "label": "API type", "unit": "type", "format": "text"},
    "consistency_level": {"category": "performance", "id": "consistency_level", "label": "Consistency level", "unit": "level", "format": "text"},
    ...
    "active_connections": {
        ...
        "unit": "connections",
        "format": "number",
        "status_fn": lambda v: "high" if v is not None and float(v) >= 3500 else None,
    },
    "max_connections": {
        "category": "performance",
        "id": "max_connections",
    ...
    "replication_lag_sec": {
        ...
        "unit": "seconds",
        "format": "number",
        "status_fn": lambda v: "high" if v is not None and float(v) > 5 else None,
    },
    "backup_storage_bytes": {
        "category": "performance",
        "id": "backup_storage_bytes",
    ...
    "ops_per_sec": {"category": "performance", "id": "ops_per_sec", "label": "Operations per second", "unit": "ops/s", "format": "number"},
    "cache_hit_rate": {"category": "performance", "id": "cache_hit_rate", "label": "Cache hit rate", "unit": "%", "format": "percent", "status_fn": lambda v: "low" if v is not None and float(v) < 50 else None},
    "cache_miss_rate_pct": {"category": "performance", "id": "cache_miss_rate", "label": "Cache miss rate", "unit": "%", "format": "percent"},
    "cache_misses": {"category": "performance", "id": "cache_misses", "label": "Cache misses", "unit": "count", "format": "number"},
    "evicted_keys": {"category": "performance", "id": "evicted_keys", "label": "Evicted keys", "unit": "count", "format": "number", "status_fn": lambda v: "high" if v is not None and float(v) > 0 else None},
    "server_load_pct": {"category": "performance", "id": "server_load", "label": "Server load", "unit": "%", "format": "percent", "status_fn": _status_cpu},
    "connected_clients": {"category": "performance", "id": "connected_clients", "label": "Connected clients", "unit": "count", "format": "number"},
}
...
RULE_METRIC_PROFILES: dict[str, tuple[str, ...]] = {
    ...
    "NAT_GATEWAY_SNAT_EXHAUSTION": ("snat_utilization_pct", "snat_connections"),
    "LOAD_BALANCER_SNAT_PRESSURE": ("snat_port_usage_pct", "used_snat_ports", "allocated_snat_ports"),
    "LOAD_BALANCER_THROUGHPUT_RIGHTSIZE": ("byte_count", "byte_count_peak"),
    "APP_GATEWAY_CU_SATURATION": ("billed_capacity_units", "cu_utilization_pct", "current_connections"),
    "APP_GATEWAY_CU_RIGHTSIZE_DOWN": ("billed_capacity_units", "cu_utilization_pct"),
    "NIC_ORPHANED_EXTENDED": ("has_vm",),
    "NIC_UNATTACHED": ("has_vm",),
    "SQL_SERVERLESS_EXTENDED": ("avg_cpu", "tier", "sku"),
    "COSMOS_AUTOSCALE_EXTENDED": ("requests", "total_ru", "normalized_ru_pct"),
    "COSMOS_SERVERLESS": ("total_ru", "requests"),
    "COSMOS_RU_RIGHT_SIZING_UNDER": ("normalized_ru_pct", "provisioned_throughput"),
    "COSMOS_RU_RIGHT_SIZING_OVER": ("normalized_ru_pct", "provisioned_throughput"),
    "COSMOS_THROTTLING_DETECTED": ("normalized_ru_pct", "normalized_ru_peak_pct"),
    "COSMOS_HOT_CONTAINER_DETECTED": ("ru_skew_ratio", "normalized_ru_peak_pct"),
    "COSMOS_API_COST_VARIANCE": ("api_type",),
    "COSMOS_CONSISTENCY_OVERPROVISIONED": ("consistency_level",),
    "COSMOS_LARGE_ITEMS_DETECTED": ("avg_item_bytes", "document_count", "data_usage_bytes"),
    "COSMOS_INDEXING_OVERPROVISIONED": ("index_to_data_ratio", "index_usage_bytes", "data_usage_bytes"),
    "COSMOS_MULTI_WRITE_UNNECESSARY": ("multi_write_enabled", "region_count"),
    "COSMOS_FAILOVER_UNNECESSARY": ("automatic_failover_enabled", "environment"),
    "COSMOS_FREE_TIER_SUBOPTIMAL": ("free_tier_enabled", "total_ru"),
    "COSMOS_RESERVED_CAPACITY_ELIGIBLE": ("normalized_ru_pct", "provisioned_throughput"),
    "STORAGE_LIFECYCLE_EXTENDED": ("transactions", "used_capacity", "access_tier"),
    "STORAGE_REDUNDANCY_EXTENDED": ("sku", "access_tier"),
    "STORAGE_EGRESS_HIGH_EXTENDED": ("egress", "used_capacity"),
    "STORAGE_COOL_TIER_CANDIDATE_EXTENDED": ("transactions", "access_tier"),
    ...
    "POSTGRESQL_LOW_COMPUTE_UTILIZATION": ("avg_cpu", "avg_memory", "tier", "sku"),
    "POSTGRESQL_HIGH_COMPUTE_DEMAND": ("avg_cpu", "tier", "sku"),
    "POSTGRESQL_MEMORY_PRESSURE": ("avg_memory", "tier", "sku"),
    "POSTGRESQL_STORAGE_EXPANSION": ("storage_gb", "storage_utilization"),
    "POSTGRESQL_IOPS_PRESSURE": ("disk_iops_pct",),
    "POSTGRESQL_CONNECTION_POOL_RISK": ("active_connections",),
    "POSTGRESQL_HA_UNNECESSARY": ("ha_mode", "environment"),
    "POSTGRESQL_HA_REQUIRED": ("ha_mode", "environment"),
    "POSTGRESQL_READ_REPLICA_ANALYSIS": ("replication_lag_sec", "replica_role"),
    "POSTGRESQL_VERSION_OUTDATED": ("version",),
    "POSTGRESQL_BACKUP_RETENTION_REVIEW": ("backup_retention_days", "backup_storage_bytes"),
    "AKS_IDLE_POOL_EXTENDED": ("cluster_cpu", "cluster_memory", "idle_nodes", "idle_node_ratio"),
    "AKS_NODE_IDLE": ("cluster_cpu", "cluster_memory", "idle_nodes", "idle_node_ratio"),
    "AKS_OVERPROVISIONED": ("cluster_cpu", "cluster_memory", "idle_nodes", "idle_node_ratio"),
    "AKS_NO_AUTOSCALER": ("cluster_cpu", "cluster_memory", "autoscaler"),
    "AKS_NO_AUTOSCALER_EXTENDED": ("cluster_cpu", "cluster_memory", "autoscaler"),
    "AKS_OLD_VERSION": ("k8s_version",),
    "AKS_OLD_VERSION_EXTENDED": ("k8s_version",),
    "AKS_NONPROD_SCHEDULING": ("cluster_cpu", "environment"),
    "AKS_NO_SPOT": ("cluster_cpu",),
    "AKS_POOL_CONSOLIDATION": ("cluster_cpu", "cluster_memory"),
    ...
    "REDIS_IDLE_DETECTION": ("ops_per_sec", "memory_usage"),
    "REDIS_MEMORY_PRESSURE": ("memory_usage", "evicted_keys"),
    "REDIS_LOW_UTILIZATION": ("memory_usage", "server_load"),
    "REDIS_HIT_RATIO_POOR": ("cache_hit_rate", "memory_usage"),
    "REDIS_CLUSTER_UNNECESSARY": ("ops_per_sec", "shard_count"),
    "REDIS_PERSISTENCE_REVIEW": ("persistence_modes", "tier"),
    "REDIS_TIER_REVIEW": ("shard_count", "maxmemory_policy", "tier"),
...
def _metric_entry(
    ...
    *,
    ...
    from app.service_display import format_service_fact, missing_display

    resolved_fact_key = fact_key or metric_id
    if unavailable or value is None:
        formatted = missing_display(canonical_type or None)
    elif fact_key:
        formatted = format_service_fact(canonical_type or None, resolved_fact_key, value, unit=unit or None)
    else:
        formatted = _format_metric_value(value, fmt)
    entry: dict[str, Any] = {
        "id": metric_id,
        "label": label,
        "unit": unit,
...
def _skip_performance_fact_key(key: str, component: str, facts: dict[str, Any]) -> bool:
    from app.rule_evidence_config import is_inventory_property_fact

    if is_inventory_property_fact(key):
        return True
...
def _rule_metric_profile(
    ...
    *,
    ...
    profile = RULE_METRIC_PROFILES.get(rid)
    if profile is not None:
        return profile
    from app.rule_evidence_config import metric_ids_from_required_evidence

    component = resource_type or ""
    json_profile = metric_ids_from_required_evidence(rid, component)
...
def build_optimization_metrics(
    ...
    *,
    ...
    rule_profile = _rule_metric_profile(
        rule_id,
        determination,
        resource_type=resource_type or str(finding.get("resource_type") or ""),
    )
    ...
    # No JSON contract: do not dump all inventory facts — cost row only (above).
