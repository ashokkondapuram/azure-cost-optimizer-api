...
def _resolve_analysis_run_id(db: Session, result: dict, job: AnalysisJob) -> str:
    """Return persisted run id from analysis result, creating a fallback run when absent."""
    run_id = result.get("run_id")
    if run_id:
        return str(run_id)

    log.warning(
        "analysis.missing_run_id",
        job_id=job.id,
        subscription_id=job.subscription_id,
        pipeline_status=result.get("pipeline_status"),
        analysis_trigger=result.get("analysis_trigger"),
    )
    from app.analysis_persist import persist_optimization_run

    return persist_optimization_run(
        db,
        subscription_id=job.subscription_id,
        profile=job.profile,
        engine_version=job.engine_version,
        result=result,
        data_source=str(result.get("data_source") or "db"),
    )
    ...
    *,
    ...
    if force:
        ...
        for active in active_jobs:
            cancel_analysis_job(db, active.id, sub)
    elif (
        db.query(AnalysisJob)
        .filter(
            AnalysisJob.subscription_id == sub,
...
def queue_post_sync_analysis(
    ...
    *,
    ...
    background_tasks.add_task(run_batch_job_safely, job.id)
    return {
        "status": "queued",
        "job_id": job.id,
...
def run_batch_job_safely(job_id: str) -> None:
    """Background-task entry point — always surfaces failures on the job record."""
    try:
        execute_batch_job(job_id)
    except Exception as exc:
        log.exception("analysis.background_task_failed", job_id=job_id, error=str(exc))
        db = SessionLocal()
        try:
            job = db.query(AnalysisJob).filter(AnalysisJob.id == job_id).first()
...
def execute_batch_job(job_id: str) -> None:
    ...
    try:
        ...
        job.run_id = _resolve_analysis_run_id(db, result, job)
        ...
        if not scoring_ran:
            try:
                ...
                decision = generate_optimization_actions(db, job.subscription_id)
                log.info(
                    "analysis.actions_decided",
                    job_id=job_id,
                    subscription_id=job.subscription_id,
                    created=decision.get("created", 0),
                    updated=decision.get("updated", 0),
                )
            except Exception as exc:
                log.warning(
                    "analysis.actions_decide_failed",
                    job_id=job_id,
                    subscription_id=job.subscription_id,
                    error=str(exc)[:500],
                )
