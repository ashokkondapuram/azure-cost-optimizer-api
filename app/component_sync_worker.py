...
def run_component_sync(component: str) -> dict[str, Any]:
    ...
    try:
        ...
        if analysis_after_component_sync() and synced:
            ...
            from app.operations_scheduler import scheduled_pipeline_enabled, run_scheduled_pipeline

            if unified_recommendation_mode() and scheduled_pipeline_enabled():
                try:
                    pipeline_result = run_scheduled_pipeline(synced)
                    pipeline_completed = list(pipeline_result.get("completed") or [])
                    analysis_completed = list(pipeline_completed)
                except Exception as exc:
                    errors.append({"subscription_id": "pipeline", "error": str(exc)[:500]})
            else:
                from app.optimizer.analysis_routing import should_chain_component_analysis

                chain_legacy = should_chain_component_analysis(component)
                for sub in synced:
                    if not chain_legacy:
                        analysis_skipped.append({
                            "subscription_id": sub,
                            "reason": "assessment_pipeline_primary",
                        })
                        continue
                    db2 = SessionLocal()
                    try:
                ...
                if scheduled_pipeline_enabled() and synced and not chain_legacy:
                    try:
                        pipeline_result = run_scheduled_pipeline(synced)
                        pipeline_completed = list(pipeline_result.get("completed") or [])
                    except Exception as exc:
            ...
            result["analysis"] = {
                ...
                "mode": "unified_pipeline" if unified_recommendation_mode() else "legacy",
            }
