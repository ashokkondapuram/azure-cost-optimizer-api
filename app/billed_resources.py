...
from app.finding_taxonomy import category_from_resource_type, format_category_label
...
def _apply_category_to_row(row: dict) -> dict:
    category = category_from_resource_type(row.get("type") or row.get("resource_type"))
    row["category"] = category
    row["categoryLabel"] = format_category_label(category)
    return row
...
def billed_row_from_inventory(
    ...
    return _apply_category_to_row(row)
...
def _filter_inventory_batch(rows: list[ResourceSnapshot]) -> list[ResourceSnapshot]:
    """Drop cost-export stubs and embedded-only resources from a small inventory batch."""
...
def _count_inventory_with_cost(db: Session, subscription_id: str, month: str) -> int:
    """Inventory rows with positive MTD cost for the month."""
    ...
    return (
        ...
        .join(
            ResourceSnapshot,
            and_(
                ResourceSnapshot.resource_id == CostByResourceSnapshot.resource_id,
                ResourceSnapshot.subscription_id == sub,
                ResourceSnapshot.is_active.is_(True),
                ResourceSnapshot.is_cost_export_only.is_(False),
...
def _fetch_inventory_cost_slice(
    db: Session,
    subscription_id: str,
    month: str,
    *,
...
def merge_billed_resources(
    ...
    *,
    ...
    return apply_costs_to_resources(merged, {}, db=db)
...
def list_billed_resources_db(db: Session, subscription_id: str) -> list[dict]:
    ...
    return merge_billed_resources(inventory, cost_map, db=db)


def _paginate_inventory_only(
    inventory: list[ResourceSnapshot],
    *,
    limit: int,
    offset: int,
    ...
    """Inventory-only page when Cost Management month is not available yet."""
    sorted_inv = sorted(inventory, key=lambda inv: (inv.resource_name or "").lower())
    total = len(sorted_inv)
    slice_inv = sorted_inv[offset:offset + limit + 1]
    rows = [billed_row_from_inventory(inv) for inv in slice_inv]
    from app.resource_store import apply_costs_to_resources

    rows = apply_costs_to_resources(rows, {}, db=db)
    page_items, has_more, page_count = slice_page(rows, limit)
    return {
        "items": page_items,
        "total": total,
        "limit": limit,
...
def list_billed_resources_page(
    ...
    *,
    ...
    if not month:
        inventory = _load_inventory_snapshots(db, subscription_id)
        return _paginate_inventory_only(inventory, limit=limit, offset=offset, db=db)

    if inventory_only:
        n_cost = _count_inventory_with_cost(db, sub, month)
    else:
        n_cost = _count_billed_with_cost(db, sub, month)
    n_pending = _count_inventory_pending_cost(db, sub, month)
    ...
    if month and offset < n_cost and want > 0:
        fetch_cost = (
            _fetch_inventory_cost_slice
            if inventory_only
            else _fetch_cost_slice
        )
        cost_rows = fetch_cost(
            db,
            sub,
            month,
            offset=offset,
        ...
        for cost in cost_rows:
            rid = normalize_arm_id(cost.resource_id)
            inv = inv_lookup.get(rid)
            if inventory_only and inv is None:
                continue
            items.append(billed_row_from_cost(cost, inv))
        want -= len(cost_rows)
    ...
    page_items = apply_costs_to_resources(page_items, {}, db=db)
