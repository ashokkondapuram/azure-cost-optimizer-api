    ...
    *,
    ...
    with arm_auth_context(db=db, token=token):
        ...
        client = cost_client_for_sync(
            subscription_id=subscription_id,
            pipeline_id=pipeline_id or f"cost-explorer:{subscription_id}",
            db=db,
            token=token,
        )
    ...
    if not export_rows and not type_rows and subscription_pretax == 0:
        log.warning(
            ...
            subscription_id=subscription_id,
            hint=(
                "Azure returned zero rows. Verify Cost Management Reader on the subscription "
                "and that ActualCost data exists for this period."
            ),
        )
        try:
            _record_cost_sync_run(
    ...
    if collector is not None:
        collector.add_section(
            "cost_meta",
            {
                ...
                "mtd_start": mtd_start,
                "mtd_end": mtd_end,
                "daily_history_start": daily_history_start,
            },
        )
        try:
            service_agg = aggregate_cost_rows_by_service(by_service_resp)
            collector.add_section("cost_by_service", service_agg)
            counts["cost_by_service"] = len(service_agg)
        except Exception as exc:
        ...
        try:
            ...
            collector.add_section("cost_by_resource_type", type_agg)
            counts["cost_by_resource_type"] = len(type_agg)
        except Exception as exc:
        ...
        try:
            ...
            counts["daily_by_service"] = len(export_rows)
        except Exception as exc:
        ...
        try:
            ...
            collector.add_section("cost_by_resource", by_resource)
            counts["cost_by_resource"] = len(by_resource)
        except Exception as exc:
        ...
        log.info("cost_explorer_sync.collected", subscription_id=subscription_id, **counts)
        return counts
    ...
    try:
        from app.budget_sync import sync_budget_snapshots

        counts["budgets"] = sync_budget_snapshots(subscription_id, db, token)
    except Exception as exc:
        write_errors.append("budgets")
        log.warning("cost_explorer_sync.budgets_failed", subscription_id=subscription_id, error=str(exc)[:200])
    ...
    try:
        ...
        db.rollback()
        log.warning("enrichment_cost_sync_failed", subscription_id=subscription_id, error=str(exc)[:200])
