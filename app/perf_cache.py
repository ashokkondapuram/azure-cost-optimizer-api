...
_health_cache: TTLCache = TTLCache(maxsize=50, ttl=900)
...
def invalidate_subscription(subscription_id: str) -> None:
    ...
    removed = 0
    with _cache_lock:
        for cache in (_cost_map_cache, _counts_cache, _findings_cache, _dashboard_cache, _health_cache):
...
def cached_azure_health_statuses(subscription_id: str, loader: Callable[[], dict]) -> dict:
    return _cached(_health_cache, subscription_id.lower(), loader)
