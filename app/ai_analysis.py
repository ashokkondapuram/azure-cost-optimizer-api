...
- implementation_steps: ordered operational steps to complete the recommendation (4–8 steps when the change is non-trivial). Each step must be specific, actionable, and grounded in the supplied evidence (metrics, checks, resource state). Use imperative verbs. Include validation checkpoints and a rollback or verification step for risky changes — not generic portal navigation.
