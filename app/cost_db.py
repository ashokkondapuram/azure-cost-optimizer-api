...
def _summary_from_resource_snapshots(
    db: Session,
    subscription_id: str,
    month: str,
) -> dict | None:
    """Sum per-resource MTD rows when subscription-level totals are unavailable."""
    sub = _normalize_sub(subscription_id)
    agg = (
        db.query(
            func.coalesce(func.sum(CostByResourceSnapshot.cost_billing), 0.0).label("pretax"),
            func.coalesce(func.sum(CostByResourceSnapshot.cost_usd), 0.0).label("usd"),
            func.max(CostByResourceSnapshot.billing_currency).label("currency"),
            func.count(CostByResourceSnapshot.id).label("row_count"),
        )
        .filter(
            CostByResourceSnapshot.subscription_id == sub,
            CostByResourceSnapshot.month == month,
    ...
    if agg and (agg.pretax or agg.usd):
        return {
            "pretax_total": round(float(agg.pretax or 0), 2),
            "cost_usd_total": round(float(agg.usd or 0), 2),
            "billing_currency": agg.currency or "CAD",
            "row_count": int(agg.row_count or 0),
            "total_source": "resource_rows_sum",
            "source": "database",
            "month": month,
        }
    ...
    inv_agg = (
        db.query(
            func.coalesce(func.sum(ResourceSnapshot.monthly_cost_billing), 0.0).label("pretax"),
            func.coalesce(func.sum(ResourceSnapshot.monthly_cost_usd), 0.0).label("usd"),
            func.max(ResourceSnapshot.billing_currency).label("currency"),
            func.count(ResourceSnapshot.id).label("row_count"),
        )
        .filter(
            ResourceSnapshot.subscription_id == sub,
            ResourceSnapshot.is_active.is_(True),
            or_(
                ResourceSnapshot.monthly_cost_billing > 0,
                ResourceSnapshot.monthly_cost_usd > 0,
            ),
        )
        .first()
    )
    if not inv_agg or (not inv_agg.pretax and not inv_agg.usd):
        return None
    return {
        "pretax_total": round(float(inv_agg.pretax or 0), 2),
        "cost_usd_total": round(float(inv_agg.usd or 0), 2),
        "billing_currency": inv_agg.currency or "CAD",
        "row_count": int(inv_agg.row_count or 0),
        "total_source": "inventory_snapshot_sum",
        "source": "database",
        "month": month,
    }
...
def cost_summary_from_db(
    ...
    *,
    ...
    period = period_for_timeframe(timeframe, from_date=from_date, to_date=to_date)
    type_filter = _expanded_resource_types(resource_types)
    default_month = month or month_for_timeframe(timeframe, from_date=from_date, to_date=to_date)
    m = month or _resolve_cost_month(db, subscription_id, tf, None) or default_month
    service_names = (
        _service_names_for_resource_types(db, sub, m, type_filter)
        if type_filter
        else None
    ...
    if tf in _CALENDAR_MTD_TIMEFRAMES or tf == "TheLastMonth":
        ...
        resource_summary = _summary_from_resource_snapshots(db, sub, m)
        if resource_summary:
            return {**resource_summary, **period_meta, "month": m}
        return None
    ...
    if daily_summary:
        return _with_period(daily_summary)
    resource_summary = _summary_from_resource_snapshots(
        db, sub, m or default_month,
    )
    if resource_summary:
        return _with_period(resource_summary)
    return None
...
def resource_daily_cost_series(
    db: Session,
    subscription_id: str,
    resource_id: str,
    *,
    ...
    """Labeled daily spend points for one resource (oldest-first)."""
    from datetime import date, timedelta
    ...
    values = resource_daily_cost_histories(db, sub, [resource_id], days=days).get(rid, [])
    end = date.today()
    start = end - timedelta(days=max(1, days) - 1)
    day_keys = [(start + timedelta(days=offset)).isoformat() for offset in range(days)]
    return [
        {
            "date": day,
            "cost": round(float(values[idx] if idx < len(values) else 0.0), 4),
        }
