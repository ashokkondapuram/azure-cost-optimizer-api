...
NAV_PANEL_CATALOG: list[dict[str, Any]] = [
  # Overview — Wiz-style unified experience
  {"id": "/action-centre", "label": "Action centre", "group": "overview", "admin_only": False},
  {"id": "/explorer", "label": "Resource inventory", "group": "overview", "admin_only": False, "superuser_only": True},
  {"id": "/dashboard", "label": "Dashboard", "group": "overview", "admin_only": False},
  ...
  {"id": "/cost-comparison", "label": "Cost comparison", "group": "advanced", "admin_only": False},
  ...
  # System
  {"id": "/admin/optimization", "label": "Sync center", "group": "system", "admin_only": True},
  {"id": "/history", "label": "Run history", "group": "system", "admin_only": False},
  {"id": "/activity-log", "label": "Activity log", "group": "system", "admin_only": False},
...
_ADVANCED_SUBGROUPS: dict[str, str] = {
  ...
  "/cost-comparison": "advanced-insights",
...
def _panel_allowed(panel_id: str, role: str, policy: dict[str, dict[str, bool]]) -> bool:
    panel = _CATALOG_BY_ID.get(panel_id)
    if panel and panel.get("superuser_only") and role != ROLE_SUPERUSER:
        return False
...
def is_path_allowed(path: str, role: str, policy: dict[str, dict[str, bool]]) -> bool:
    ...
    if normalized.startswith("/optimization-hub"):
        normalized = "/action-centre"

    for prefix in ("/action-centre", "/explorer"):
        if normalized == prefix or normalized.startswith(f"{prefix}/"):
            return _path_allowed(prefix, role, policy)

    # Legacy resource inventory routes redirect to Action centre in the SPA.
    if normalized in _RESOURCE_SUBGROUPS:
        subgroup = _RESOURCE_SUBGROUPS[normalized]
        if not _panel_allowed(f"section:resources:{subgroup}", role, policy):
            return False
        return _path_allowed("/action-centre", role, policy)
