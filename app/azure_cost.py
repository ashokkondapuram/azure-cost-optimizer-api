...
from app.auth import auth_headers, current_arm_auth, fresh_azure_headers, refresh_azure_token
...
_COLUMN_ALIASES = {
    ...
    # ActualCost / FOCUS exports often return these instead of PreTaxCost.
    "Cost": "PreTaxCost",
    "BilledCost": "PreTaxCost",
    ...
    for attempt in range(1, _MAX_5XX_RETRIES + 1):
        try:
            ...
            if exc.status == 401 and not _retried:
                ...
                ctx = current_arm_auth()
                effective_db = ctx.get("db")
                refresh_azure_token(effective_db)
                clear_arm_cache()
                fresh = fresh_azure_headers(effective_db)
...
class AzureCostClient:
    ...
    def query_subscription_totals(
        ...
        *,
        ...
        # Canonical column order — do not reuse Azure column order (Currency may be first).
        canonical_columns = [
            {"name": "PreTaxCost"},
            {"name": "CostUSD"},
            {"name": "Currency"},
