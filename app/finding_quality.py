...
from app.assessment.governance_filter import is_region_governance_finding
from app.inventory_standalone import is_embedded_only_arm_id, is_standalone_inventory_type
...
def is_valuable_finding(
    ...
    *,
    ...
    if rule_id in _LOW_VALUE_GOVERNANCE_RULES and savings <= 0:
        return False
    if is_region_governance_finding(
        finding if isinstance(finding, dict) else {"rule_id": rule_id, "category": category, "evidence": evidence},
        evidence,
    ):
        return False
...
def _finding_canonical_type(finding: Any) -> str:
    resource_type = str(_field(finding, "resource_type") or "").strip().lower()
    if resource_type:
        return resource_type
    resource_id = str(_field(finding, "resource_id") or "").strip()
    return (internal_resource_type(resource_id) or "").strip().lower()


def is_action_centre_finding(finding: Any) -> bool:
    """True when a finding should surface in Action centre as a user-facing recommendation."""
    if is_embedded_only_arm_id(str(_field(finding, "resource_id") or "")):
        return False
    if not is_standalone_inventory_type(_finding_canonical_type(finding)):
        return False
    rule_id = str(_field(finding, "rule_id") or "")
    if rule_id.startswith("metric_") and "_missing" in rule_id:
        return False
    if "_baseline_missing" in rule_id:
        return False
    evidence = parse_finding_evidence(finding)
    if is_region_governance_finding(
        finding if isinstance(finding, dict) else {"rule_id": rule_id, "evidence": evidence},
        evidence,
    ):
        return False
    return True
...
def filter_action_centre_findings(findings: list[Any]) -> list[Any]:
    return [f for f in findings if is_action_centre_finding(f)]
