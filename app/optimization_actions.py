...
def _action_summary(db: Session, subscription_id: str) -> dict[str, int]:
    ...
    rows = (
        ...
        .group_by(OptimizationAction.workflow_status)
        .all()
    )
...
def list_optimization_actions(
    ...
    *,
    ...
    sub = subscription_id.strip().lower()
    q = db.query(OptimizationAction).filter(OptimizationAction.subscription_id == sub)
