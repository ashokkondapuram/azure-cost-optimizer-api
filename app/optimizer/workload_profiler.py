...
from app.models import ResourceSnapshot, WorkloadProfile
...
def _history_values(
    ...
    *,
    ...
    """Legacy utilization history removed — use current cached facts only."""
    facts = load_cached_resource_facts(db, resource_id) or {}
    value = facts.get(metric_name)
    if value is None:
        return []
    try:
        return [float(value)]
