...
from app.technical_fetch_specs import (
    ...
    *,
    ...
    if metrics or monitor_facts:
        ...
        for key, val in monitor_facts.items():
            runtime[key] = val
        for key in (
            "avg_cpu_pct", "avg_mem_pct", "avg_memory_pct", "cpu_pct", "memory_pct",
            "cluster_cpu_pct", "cluster_mem_pct", "node_cpu_pct", "node_mem_pct",
            "storage_pct", "memory_pct", "used_capacity_bytes", "transaction_count",
        ):
            if key in facts:
...
def merge_envelope_into_evidence(
    ...
    if not envelope:
        return evidence
    merged = dict(envelope.facts)
    merged.update(evidence)
    merged["resource_elements"] = envelope.elements
    merged["sub_engine"] = envelope.canonical_type
    return merged
