...
def _monthly_cost_from_snap(snap: ResourceSnapshot | None) -> float:
    if not snap:
        return 0.0
    billing = float(snap.monthly_cost_billing or 0)
    usd = float(snap.monthly_cost_usd or 0)
...
def analyze_resource_trends(
    ...
    monthly_cost = _monthly_cost_from_snap(snap)
