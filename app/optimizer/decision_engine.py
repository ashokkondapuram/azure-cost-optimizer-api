...
from app.recommendation_output import synthesize_action_narrative
...
_ACTION_REASONS: dict[str, str] = {
    "resize_down": "CPU and memory utilization are below the rightsizing threshold — reduce VM SKU to match observed load",
    "downgrade_disk": "Disk tier or size exceeds what utilization and I/O patterns require",
    "decommission": "Resource is idle or unused — remove it to stop recurring charges",
    "buy_reservation": "Spend is stable enough to cover with a reservation or savings plan",
    "investigate": "Cost or utilization signals need review before any change is approved",
    "manual_review": "Potential savings conflict with performance, SLA, or dependency risk — validate before acting",
    "keep": "Signals do not justify a change right now",
}
...
def _finding_to_narrative_dict(finding: OptimizationFinding) -> dict[str, Any]:
    """Convert an ORM finding row into a dict for narrative synthesis."""
...
def _synthesize_action_reason(
    *,
    ...
    finding_dicts = [_finding_to_narrative_dict(f) for f in findings]
    narrative_result = synthesize_action_narrative(
        finding_dicts,
        action_type=action,
        estimated_savings=scorecard.get("cost_savings_monthly"),
        workload_type=workload.get("workload_type"),
        recommendation_tier=scorecard.get("recommendation_tier"),
        fallback_reason=base,
    )
    return narrative_result.get("narrative") or base, narrative_result.get("highlights") or []
...
def _build_analysis_payload(
    *,
    ...
    monthly_cost = monthly_cost_from_snapshot(snapshot)
    if monthly_cost <= 0:
        for finding in findings:
            ev = _finding_evidence(finding)
...
def _decide_for_resource(
    *,
    ...
    action_reason, narrative_highlights = _synthesize_action_reason(
        scorecard=scorecard,
        findings=open_findings,
        workload=workload,
    )
    if narrative_highlights:
        utilization_evidence["narrative_highlights"] = narrative_highlights
