...
def known_analysis_components() -> set[str]:
    return {b["component"] for b in ANALYSIS_BATCHES}


def filter_known_components(components: list[str] | None) -> list[str]:
    """Drop legacy/removed component labels (e.g. standalone VMSS) from job scope."""
    if not components:
        return []
    known = known_analysis_components()
    return [c for c in components if c in known]
...
def resolve_batches(components: list[str] | None = None) -> list[dict]:
    ...
    want = set(filter_known_components(components))
    if not want:
        return []
    return [b for b in ANALYSIS_BATCHES if b["component"] in want]
