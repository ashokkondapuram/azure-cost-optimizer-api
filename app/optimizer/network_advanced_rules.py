...
def analyze_network_advanced(
    ...
    fd_rule = engine.rules.get("NETWORK_FRONT_DOOR_REVIEW")
    ...
    for resource in resources_by_type.get("network/frontdoor", []):
        if not fd_rule or not fd_rule.enabled:
            break
        monthly = resource_cost(cost_by_resource or {}, resource.get("id", ""))
        if monthly >= getattr(fd_rule, "min_monthly_savings_usd", 10.0):
            out.append(Finding(
                fd_rule, resource,
                detail=f"Front Door profile '{resource.get('name')}' has recurring cost — review routing rules and WAF tier.",
                recommendation="Consolidate entry points or downgrade unused WAF policies.",
                savings=round(monthly * 0.20, 2),
                score=42,
