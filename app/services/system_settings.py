...
def _coerce_value(key: str, value: Any) -> Any:
    if key in {"port", "request_timeout_seconds"}:
        try:
            return int(value)
    ...
    if key in {"ai_enabled", "ai_enrich_all_findings"}:
        if isinstance(value, bool):
            return value
        return str(value).lower() in {"1", "true", "yes", "on"}
    if key in {"ai_max_findings_per_run", "ai_batch_size"}:
        try:
            return int(value)
        except (TypeError, ValueError):
            return value
...
def _env_value(category: str, field: str) -> str | None:
    ...
    if env_name:
        ...
        if val is not None and val != "":
            return val
    if category == "ai" and field == "openai_key":
