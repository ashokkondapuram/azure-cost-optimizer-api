...
class CustomBudget(Base):
    """App-defined monthly budget thresholds (persisted)."""
    __tablename__ = "custom_budgets"
    __table_args__ = (
        Index("ix_custom_budgets_sub", "subscription_id"),
    )

    id                   = Column(String, primary_key=True)
    subscription_id      = Column(String, nullable=False)
    name                 = Column(String, nullable=False)
    scope                = Column(String, default="subscription")
    monthly_limit        = Column(Float, nullable=False)
    currency             = Column(String, default="CAD")
    period               = Column(String, default="monthly")
    alert_thresholds_json = Column(Text, default="[80.0]")
    created_at           = Column(DateTime(timezone=True), default=_now)
...
class K8sClusterConnection(Base):
    """Dashboard-connected AKS cluster for in-cluster metrics agent deployment."""
    __tablename__ = "k8s_cluster_connections"
    __table_args__ = (
        UniqueConstraint(
            "subscription_id", "resource_group", "cluster_name",
            name="uq_k8s_cluster_conn",
        ),
        Index("ix_k8s_cluster_conn_name", "cluster_name"),
    )
    id                  = Column(String, primary_key=True)
    subscription_id     = Column(String, nullable=False)
    resource_group      = Column(String, nullable=False)
    cluster_name        = Column(String, nullable=False)
    arm_id              = Column(String, nullable=True)
    location            = Column(String, nullable=True)
    kubernetes_version  = Column(String, nullable=True)
    agent_deployed      = Column(Boolean, default=False)
    agent_status        = Column(String, default="not_deployed")
    agent_deployed_at   = Column(DateTime(timezone=True), nullable=True)
...
class AnalysisJob(Base):
    ...
    stage_results_json = Column(Text, default="{}")
    created_at        = Column(DateTime(timezone=True), default=_now)
...
class ResourceSkuPricing(Base):
    """
    Unified Azure retail / catalog monthly price cache for all resource SKUs.

    Global rows (subscription_id NULL) are shared across subscriptions.
    Lookup key: canonical_type + region + arm_sku_name + optional capacity_gb + os_type.
    """
    __tablename__ = "resource_sku_pricing"
    __table_args__ = (
        UniqueConstraint("lookup_key", name="uq_rsp_lookup_key"),
        Index("ix_rsp_canonical_region", "canonical_type", "region"),
        Index("ix_rsp_expires", "expires_at"),
        Index("ix_rsp_fetched", "fetched_at"),
    )

    id               = Column(String, primary_key=True)
    subscription_id  = Column(String, nullable=True)
    canonical_type   = Column(String, nullable=False)
    arm_sku_name     = Column(String, nullable=False)
    sku_name         = Column(String)
    region           = Column(String, nullable=False)
    capacity_gb      = Column(Integer, nullable=True)
    ...
    sku_details_json = Column(Text, default="{}")
...
# ---------------------------------------------------------------------------
# Assessment pipeline — quality scores (snapshots live in per-type enrichment)
# ---------------------------------------------------------------------------

# Per-type enrichment tables are created dynamically via
# ``app.data_store.enrichment_registry`` (``resource_enrichment_{type_slug}``).


class ResourceAssessmentResult(Base):
    """Data quality score and matched conditions from assessment pythonAssessment."""
    __tablename__ = "resource_assessment_results"
    __table_args__ = (
        UniqueConstraint("subscription_id", "resource_id", name="uq_rar_sub_resource"),
        Index("ix_rar_sub_classification", "subscription_id", "classification"),
    )
    ...
    resource_id      = Column(String, nullable=False)
    resource_type    = Column(String, nullable=False)
    assessment_file  = Column(String, nullable=True)
    score            = Column(Float, default=0.0)
    classification   = Column(String, default="unknown")
    data_quality_json = deferred(Column(JSONText, default="{}"))
    assessed_at      = Column(DateTime(timezone=True), default=_now, onupdate=_now)


class PipelineRun(Base):
    """Tracks assessment pipeline execution per subscription."""
    __tablename__ = "pipeline_runs"
    __table_args__ = (
        Index("ix_pr_sub_status", "subscription_id", "status"),
    ...
    stage_results_json = Column(JSONText, default="{}")
    error_message     = Column(Text, nullable=True)
    created_at        = Column(DateTime(timezone=True), default=_now)
    started_at        = Column(DateTime(timezone=True), nullable=True)
    completed_at      = Column(DateTime(timezone=True), nullable=True)
...
class FullSyncPipelineRun(Base):
    """Unified inventory → cost → metrics → analysis pipeline (survives restarts)."""
    __tablename__ = "full_sync_pipeline_runs"
    __table_args__ = (
        Index("ix_fsp_sub_created", "subscription_id", "created_at"),
        Index("ix_fsp_sub_status", "subscription_id", "status"),
    )
    ...
    current_stage    = Column(String, nullable=True)
    progress_pct     = Column(Integer, default=0)
    stages_json      = Column(JSONText, default="{}")
