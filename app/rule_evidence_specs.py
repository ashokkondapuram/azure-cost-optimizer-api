...
RULE_EVIDENCE_SPECS: dict[str, RuleEvidenceSpec] = {
    ...
    "AKS_NODE_IDLE": _spec(
        ...
        signals=(
            ...
            SignalDef("Pool node count", "node_count", comparator="not_empty"),
        ),
        summary="{idle_nodes} of {node_count} nodes in pool '{pool_name}' are idle.",
    ...
    "AKS_OVERPROVISIONED": _spec(
        "idle_nodes",
        signals=(SignalDef("Idle nodes", "idle_nodes", threshold_literal="0", comparator="gt"),),
    ...
    "AKS_OLD_VERSION": _spec(
        "unsupported_k8s",
        signals=(SignalDef("Kubernetes version", "kubernetes_version", comparator="not_empty"),),
    ...
    "AKS_NO_AUTOSCALER": _spec(
        ...
        signals=(
            ...
            SignalDef("Node count", "node_count", threshold_key="node_count_min", comparator="gt"),
        ),
        summary="Pool '{pool_name}' has {node_count} nodes without cluster autoscaler.",
        savings_method="factor_of_monthly_cost",
        savings_factor=0.30,
    ),
    ...
    "STORAGE_HOT_UNUSED": _spec(
        "hot_tier_review",
        signals=(SignalDef("Access tier", "access_tier", threshold_literal="Hot", comparator="eq"),),
        summary="Storage account access tier is Hot — verify active access patterns.",
        savings_method="governance",
    ),
    "STORAGE_NO_LIFECYCLE": _spec(
        "no_lifecycle_policy",
        signals=(SignalDef("Lifecycle policy", "has_lifecycle_policy", threshold_literal="configured", comparator="falsy"),),
        summary="No lifecycle management policy verified for this storage account.",
        savings_method="governance",
    ),
    "STORAGE_LRS_CRITICAL": _spec(
        "lrs_critical_data",
        signals=(SignalDef("Redundancy SKU", "sku", threshold_literal="GRS/GZRS", comparator="neq"),),
        summary="Critical data may need geo-redundant storage (current SKU: {sku}).",
        savings_method="governance",
    ),
    "STORAGE_LIFECYCLE_EXTENDED": _spec(
        "no_lifecycle_policy",
        signals=(SignalDef("Lifecycle policy", "has_lifecycle_policy", threshold_literal="configured", comparator="falsy"),),
        summary="Add lifecycle rules to move data to Cool/Archive tiers.",
        savings_method="factor_of_monthly_cost",
        savings_factor=0.25,
    ),
    "STORAGE_REDUNDANCY_EXTENDED": _spec(
        ...
        signals=(
            SignalDef("Redundancy SKU", "sku", comparator="not_empty"),
            SignalDef("Environment", "environment", comparator="not_empty"),
        ),
        summary="Non-prod storage uses geo-redundant SKU {sku}.",
        savings_method="factor_of_monthly_cost",
        savings_factor=0.50,
    ),
    # ── Network ──────────────────────────────────────────────────────────
...
def build_checks(spec: RuleEvidenceSpec, facts: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for sig in spec.signals:
        ...
        checks.append({
            "signal": sig.signal,
            "value": display if display is not None and display != "" else "—",
            "threshold": threshold,
            "passed": passed,
