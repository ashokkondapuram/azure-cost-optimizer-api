...
def _filter_azure_inventory_rows(rows: list) -> list:
    ...
    return [row for row in rows if not getattr(row, "is_cost_export_only", False)]
...
def rows_to_list(rows, *, include_properties: bool = True) -> list[dict]:
    ...
    for r in deduped.values():
        ...
        row = {
            ...
            "azureServiceName": r.azure_service_name,
...
def apply_costs_to_resources(
    ...
    *,
    ...
    """Overlay MTD, lifetime, and trend costs onto resource list rows."""
    ...
    for row in rows:
        ...
        if detail:
            pretax, usd, currency = monthly_cost_amounts_from_entry(detail)
            if pretax > 0 or usd > 0:
                row["monthlyCostBilling"] = pretax
                row["monthlyCostUsd"] = usd
                row["billingCurrency"] = currency or row.get("billingCurrency") or "CAD"
