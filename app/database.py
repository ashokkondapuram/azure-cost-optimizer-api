...
def _make_engine(url: str):
    eng = create_engine(
        ...
        pool_pre_ping=True,
        pool_size=10 if not url.startswith("sqlite") else 1,
...
def migrate_schema():
    ...
    if not insp.has_table("resource_utilization_history"):
        from .models import ResourceUtilizationHistory
        ResourceUtilizationHistory.__table__.create(bind=engine, checkfirst=True)
