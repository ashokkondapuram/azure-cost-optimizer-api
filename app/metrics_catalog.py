...
def build_unified_metric_row(
    ...
    row["formatted"] = format_fact_display_value(fact_key, value, row.get("unit"))
    series_points = detail_row.get("series_points")
    if series_points:
    ...
    *,
    ...
    if ctype in {"compute/vm", "compute/vmss"} and facts.get("avg_memory_pct") is not None:
        derived.append({
            ...
            "unit": "percent",
            "formatted": format_fact_display_value("avg_memory_pct", facts["avg_memory_pct"], "percent"),
    ...
    if ctype == "storage/account" and facts.get("storage_pct") is not None:
        derived.append({
            ...
            "unit": "percent",
            "formatted": format_fact_display_value("storage_pct", facts["storage_pct"], "percent"),
    ...
    if ctype == "compute/disk" and facts.get("disk_iops_utilization_pct") is not None:
        derived.append({
            ...
            "unit": "percent",
            "formatted": format_fact_display_value(
                "disk_iops_utilization_pct", facts["disk_iops_utilization_pct"], "percent",
            ),
    ...
    if ctype == "compute/disk" and facts.get("disk_throughput_utilization_pct") is not None:
        derived.append({
            ...
            "unit": "percent",
            "formatted": format_fact_display_value(
                "disk_throughput_utilization_pct", facts["disk_throughput_utilization_pct"], "percent",
            ),
    ...
    if ctype == "network/nat" and facts.get("snat_utilization_pct") is not None:
        derived.append({
            "fact_key": "snat_utilization_pct",
            "label": "SNAT utilization",
            "value": facts["snat_utilization_pct"],
            "unit": "percent",
            "formatted": format_fact_display_value("snat_utilization_pct", facts["snat_utilization_pct"], "percent"),
            "source": "computed_from_snat_connections_and_capacity",
            "impact": "performance",
            "status": _metric_def_status("snat_utilization_pct", facts["snat_utilization_pct"]),
        })
    if ctype == "network/loadbalancer" and facts.get("snat_port_usage_pct") is not None:
        derived.append({
            "fact_key": "snat_port_usage_pct",
            "label": "SNAT port usage",
            "value": facts["snat_port_usage_pct"],
            "unit": "percent",
            "formatted": format_fact_display_value("snat_port_usage_pct", facts["snat_port_usage_pct"], "percent"),
            "source": "computed_from_used_and_allocated_snat_ports",
            "impact": "performance",
            "status": _metric_def_status("snat_port_usage_pct", facts["snat_port_usage_pct"]),
        })
    if ctype == "network/appgateway" and facts.get("cu_utilization_pct") is not None:
        derived.append({
            "fact_key": "cu_utilization_pct",
            "label": "Capacity unit utilization",
            "value": facts["cu_utilization_pct"],
            "unit": "percent",
            "formatted": format_fact_display_value("cu_utilization_pct", facts["cu_utilization_pct"], "percent"),
            "source": "computed_from_billed_cu_and_provisioned_capacity",
            "impact": "both",
            "status": _metric_def_status("cu_utilization_pct", facts["cu_utilization_pct"]),
        })
    return derived
