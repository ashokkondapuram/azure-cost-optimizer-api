...
def get_azure_token(db=None) -> str:
    """Unified Azure ARM bearer token from DB settings + ``azure_token_cache``."""
    return _get_bearer_token(db, scope=ARM_SCOPE)


def get_token(db=None) -> str:
    """ARM management API bearer token (alias for :func:`get_azure_token`)."""
    return get_azure_token(db)


def _sync_arm_auth_context_token(token: str, db=None) -> None:
    """Update pinned bearer token when callers are inside :func:`arm_auth_context`."""
    ctx = current_arm_auth()
    if ctx.get("token") is not None or ctx.get("db") is not None:
        _arm_auth_ctx.set({
            **ctx,
            "token": token,
            "db": db if db is not None else ctx.get("db"),
        })


def refresh_azure_token(db=None) -> str:
    """Force credential reload and return a fresh bearer token (401 recovery)."""
    ctx = current_arm_auth()
    effective_db = db if db is not None else ctx.get("db")
    reload_credential(effective_db)
    token = get_azure_token(effective_db)
    _sync_arm_auth_context_token(token, effective_db)
    return token


def get_azure_headers(db: Optional[object] = None) -> dict:
    """Authorization headers for Azure management APIs (Cost, ARM, Monitor)."""
    return auth_headers(db)


def fresh_azure_headers(db: Optional[object] = None) -> dict:
    """Headers resolved from DB cache — ignores a stale pinned context token."""
    ctx = current_arm_auth()
    effective_db = db if db is not None else ctx.get("db")
    return {
        "Authorization": f"Bearer {get_azure_token(effective_db)}",
        "Content-Type": "application/json",
    }


def cached_arm_token_available(db=None) -> bool:
    """True when a management token is already cached (no Azure AD fetch)."""
    ctx = current_arm_auth()
    if ctx.get("token"):
        return True
    try:
        with scoped_session(db) as session:
            config = resolve_auth_config(session)
            cache_key = credential_cache_key(config, ARM_SCOPE)
            if _cache_hit(cache_key):
                return True
            return read_cached_token(session, cache_key) is not None
    except Exception:
        return False
