...
def load_analysis_metrics(
    ...
    *,
    ...
    def _load_k8s() -> dict[str, dict]:
        ...
        try:
            return load_k8s_node_metrics(session, buckets.get("aks_clusters") or [])
...
def load_cached_resource_facts(db: Session, subscription_id: str) -> dict[str, dict[str, float]]:
    ...
    Sources: enrichment metrics (inventory worker), open findings evidence,
    then the latest optimization run snapshot.
    """
    ...
    try:
        ...
        for rid, facts in (load_pipeline_resource_facts(db, sub) or {}).items():
            bucket = out.setdefault(rid.lower(), {})
            bucket.update(facts)
    except Exception as exc:
        log.warning("metrics_loader.pipeline_facts_failed", error=str(exc)[:200])
...
def analysis_metrics_summary(
    ...
    if monitor_stats:
        summary["monitor_fetch"] = {
            k: monitor_stats[k]
            for k in ("requested", "loaded", "empty", "failed", "auth_failed", "timed_out", "retried", "errors", "source")
