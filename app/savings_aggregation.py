...
def aggregate_subscription_savings(
    ...
    *,
    ...
    by_resource_unified: dict[str, float] = {}
    overlap_resources = 0
    ...
    for rid in resource_ids:
        ...
        if breakdown.unified_monthly > 0:
            by_resource_unified[rid] = round(float(breakdown.unified_monthly), 2)
    ...
    return {
        ...
        "by_resource_unified_monthly": by_resource_unified,
...
def unified_savings_by_resource_engine(findings: Iterable[Any]) -> dict[str, float]:
    """Engine-only unified monthly savings per resource (open findings, deduped)."""
    ...
    for rid, rows in by_resource.items():
        breakdown = resolve_resource_savings(resource_id=rid, findings=rows)
        amount = round(float(breakdown.unified_monthly or 0), 2)
        if amount > 0:
            result[rid] = amount
    return result
