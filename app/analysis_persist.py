...
def persist_optimization_run(
    ...
    *,
    ...
    findings = dedupe_finding_dicts(dedupe_commitment_findings([
    ...
    if scope_resource_types is not None:
        _normalize_and_collapse_open_findings(db, sub, now=now)

    update_resource_analysis_summaries(
        db, sub, findings, run_id, data_source, scope_resource_types=scope_resource_types,
    )
    db.commit()
    return run_id
