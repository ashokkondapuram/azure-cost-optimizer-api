...
def token_expire_delta() -> timedelta:
    ...
    if settings.jwt_expire_hours is not None:
        return timedelta(hours=settings.jwt_expire_hours)
...
def session_idle_minutes() -> int:
    ...
    return get_settings().session_idle_minutes
...
_USER_ACTIVE_CACHE: dict[str, tuple[float, Optional[dict[str, Any]]]] = {}


def auth_user_cache_ttl_seconds() -> int:
    """Short-lived cache for middleware user lookups (0 disables)."""
    return get_settings().auth_user_cache_seconds


def clear_auth_user_cache(user_id: str | None = None) -> None:
    if user_id:
...
def _coerce_password(password: str | SecretStr) -> str:
    """Normalize plain str or Pydantic SecretStr to a str for hashing."""
...
def hash_password(password: str | SecretStr) -> str:
    plaintext = _coerce_password(password)
    salt = secrets.token_hex(16)
    digest = hashlib.pbkdf2_hmac("sha256", plaintext.encode(), salt.encode(), 260_000)
    return f"{salt}${digest.hex()}"


def verify_password(password: str | SecretStr, stored_hash: str) -> bool:
    plaintext = _coerce_password(password)
    try:
        salt, digest = stored_hash.split("$", 1)
    except ValueError:
        return False
    check = hashlib.pbkdf2_hmac("sha256", plaintext.encode(), salt.encode(), 260_000).hex()
...
def resolve_authenticated_user(
    ...
    *,
    ...
    """Resolve an active app user from a decoded JWT payload.

    Results are cached briefly to avoid opening a DB connection on every request.
    """
    ...
    if db is None:
        from app.auth import scoped_session

        with scoped_session() as session:
            return _load_authenticated_user(session, user_id, ttl=ttl, cached_at=now)

    return _load_authenticated_user(db, user_id, ttl=ttl, cached_at=now)
...
def _load_authenticated_user(
    ...
    *,
    ...
    if ttl > 0:
        _USER_ACTIVE_CACHE[user_id] = (cached_at + ttl, user)
...
def _bootstrap_password(
    settings,
    *,
    env_password: str | SecretStr,
    dev_default: str,
    account: str,
) -> str | None:
    password = _coerce_password(env_password).strip() if env_password else ""
    if not password:
