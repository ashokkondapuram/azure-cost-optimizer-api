...
ENRICH_IF_MISSING: dict[str, tuple[str, ...]] = {
    ...
    "containers/aks": ("nodeProvisioningProfile", "nodeResourceGroup"),
...
def enrich_arm_resources_for_type(
    ...
    """AKS: ARM GET enrichment first, then NAP/VMSS pool merge (needs full cluster props)."""
    canonical = (canonical_type or "").strip().lower()
    if canonical == "containers/aks":
        from app.db_sync import enrich_aks_arm_clusters

        rows = enrich_arm_resources(client, subscription_id, list(resources or []), canonical)
        return enrich_aks_arm_clusters(client, subscription_id, rows)
