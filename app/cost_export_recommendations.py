...
def _is_billing_or_commitment_row(row: dict[str, Any]) -> bool:
    """Savings plans, reservations, and billing-benefit orders are not workload resources."""
    arm = _arm_provider(row)
    if arm.startswith("microsoft.billingbenefits/"):
        return True
    rid = normalize_arm_id(row.get("id") or "").lower()
    return "/microsoft.billingbenefits/" in rid
...
COST_EXPORT_RULES: list[CostExportRule] = [
    CostExportRule(
        ...
        match=lambda row: not _is_billing_or_commitment_row(row),
