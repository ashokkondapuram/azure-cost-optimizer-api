#!/usr/bin/env python3
"""Generate per-resource microservice folders from the monolith registry."""
...
ENV PORT={port}
ENV MANAGED_DISK_SPEC_PATH=/app/it_services/compute_disk/data/managed_disk_metrics_thresholds.json
...
def write_gateway_routes(configs) -> Path:
    ...
    for cfg in configs:
        routes.append({
            ...
            "target": f"http://{cfg.service_id}:{cfg.port}" if cfg.migrated else "${MONOLITH_URL}",
